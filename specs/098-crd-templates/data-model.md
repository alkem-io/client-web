...
type TemplateContent =
  | { type: 'callout';
      ...
      references?: { id: string; name: string; uri: string; description?: string }[];  // `framing.profile.references` — the *calloutReferences* the template editor produces. Rendered as a list of external links in the Callout preview (mirrors legacy MUI `<References compact />` and the in-feed `CalloutDetailDialog`'s `ReferencesAndTagsStrip`). D19, 2026-05-18.
...
| `framing.profile.references` (`ReferenceDetails`) | `TemplateContent.callout.references` | `templateContentMapper.ts → mapCalloutContent` — pure pick of `{ id, name, uri, description }` per reference (D19, 2026-05-18). Rendered in `CalloutTemplatePreview` via the existing `ReferencesAndTagsStrip` (the same one `CalloutDetailDialog` uses in the feed) — one component, identical styling between Preview dialog and in-feed callout. |
| `TemplateContent.post.defaultDescription` / `TemplateContent.whiteboard.whiteboardContent` (picker selection in *Posts defaults* dialog) | `ResponseDefaultsDialog.draft.{postDescription, whiteboardContent}` | `ResponseDefaultsConnector` (D20, 2026-05-19) — the connector receives an `applyDraft(next: Partial<ContributionDefaults>)` helper via the dialog's render-prop `templateSlot` and pushes the picked template's content straight into the dialog's local draft state. **Does not** call the parent `onSave` for the apply step — that bypassed the dialog draft and (a) didn't populate the input fields visibly, (b) was overwritten by the dialog's stale draft on Save, and (c) leaked to the parent even on Cancel. The dialog's own Save still commits draft → parent via `onSave(draft)` as before. |
...
| `space.templatesManager.templatesSet.id` (holder Space) + its `SPACE_SUBSPACE` `TemplateDefault` | `TemplatePickerSource(key='space')` + pre-selected template for subspace creation | `useCreateSubspace({ templatesSetId, accountId, defaultTemplateId })` → `useTemplatePicker({ spaceTemplatesSetId })`. The id is sourced via `useSpaceTemplatesManagerQuery` (Settings + the L0 Space-view Subspaces tab) or read off the already-fetched `SubspacePage` query (the Subspace page). **Every** create-subspace callsite must pass these options (D21, 2026-05-20) — omitting them drops the **Space** (and, at L0, **Account**) sources and the default pre-selection, silently leaving the picker Platform-only. |

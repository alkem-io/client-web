...
- `src/main/crdPages/templates/useTemplateForms.ts` — form state + validation + `useCreateTemplate*` / `useUpdate*` mutations + image uploads (`useHandlePreviewImages`, `useUploadMediaGalleryVisuals`) + the URL-resolver + space-content + Update-privilege Apollo plumbing for the Space form's URL-paste source picker (mirrors the legacy MUI `SpaceContentFromSpaceUrlForm`: paste a space URL → "Use this space" button → resolve via `useUrlResolverLazyQuery` → fetch innovation flow + callout titles via `useSpaceTemplateContentLazyQuery` → require Update privilege on the resolved space); assembles the `perTypeFormSlot` per type; reuse the MUI-free `src/domain/templates/components/Forms/common/mappings.ts`. Wire create/edit into `useTemplatesManager` (replace the existing `onCreateTemplate` TODO).
...
- `src/crd/components/innovationPack/{InnovationPackForm,InnovationPackCard,CreateInnovationPackDialog,InnovationPackAdminView,InnovationPackProfileView}.tsx` per `contracts/innovation-pack.ts`. `InnovationPackForm` = name / description / avatar / tags / references / `listedInStore` / `searchVisibility`, **provider read-only (no org picker)**. `CreateInnovationPackDialog` = **name + description only** (mirroring the legacy create dialog). `InnovationPackAdminView` = `InnovationPackForm` + `<TemplatesManagerView holderKind="innovationPack" canImport={() => false} .../>` — **no delete-pack here**. `InnovationPackProfileView` follows the prototype's `TemplatePackDetail`: a **compact header** (64px rounded thumbnail [pack avatar or `pickColorFromId` gradient + Package fallback] + name `text-page-title` + markdown description + a meta row `{N} templates` badge · `by {provider}` link · tag chips) with `ShareButton` + a "Manage pack" cogwheel (`Button variant="secondary" size="icon"` + `Settings`, matching the User/Org/VC profile heroes) on the right — **no banner, no overlapping avatar, no sidebar, no references**. Body = the holder-agnostic `TemplatesManagerView` rendered **full-width + `readOnly`** (read-only cards = preview-only kebab). The total-count badge is computed from the templates sections. The **public-profile page** wraps the view in the shared profile-page width `container mx-auto px-4 md:px-8` (same width as the User/Org/VC public profiles), **not** the `max-w-6xl` admin/library wrapper — otherwise the pack profile renders narrower than its peer profile pages.

...
- [X] T001 Verify deps installed and baseline gates pass in the worktree: `pnpm install` (if needed), `pnpm vitest run` (record baseline pass count), `pnpm lint`. Baseline: 257 test files / 2192 passed / 2 skipped (under `LC_ALL=en_US.UTF-8` — the bg_BG-locale budgetMeter test is a known pre-existing flake under other locales, unrelated to this feature); `pnpm lint` exit 0, 142 pre-existing warnings / 6 infos / 0 errors.
...
- [X] T018 [US3] Verified by code trace + existing coverage rather than a live browser session (no dev server was started per workspace policy — Polina boots the app herself): `DocumentContributionAddConnector.handleFileChange` calls `validateCollaboraImportFile([next])` synchronously and only ever calls `setFile(next)` (which gates `handleSubmit`'s `importDocument(...)` call) on `validation.ok === true`; there is no code path from an invalid file to the mutation. The exact `.pdf`-extension-rejection and >15MB-size-rejection scenarios this task asks about are already exercised by the pre-existing, untouched `validateCollaboraImportFile.spec.ts` (19 tests, including "rejects a file one byte over the cap" and unsupported-extension cases) — this connector calls that same, unmodified function. `errorMessage` (rendered via `DocumentImportZone`'s `role="alert"`) comes from `deriveCollaboraImportErrorMessage`, confirmed human-readable (T004). Dialog-stays-open is structural: no `handleClose()` call exists on the pre-check-failure path.
- [X] T019 [US3] Confirmed by reading `DocumentContributionAddConnector`'s catch block (T015): a server-side `FORMAT_NOT_SUPPORTED` rejection sets `importError` to `{ kind: 'extension', received: '' }`, which `deriveCollaboraImportErrorMessage` renders via the same `documentImportErrorUnsupported` i18n key as the client pre-check — never the raw Apollo/GraphQL error string. No fix needed.

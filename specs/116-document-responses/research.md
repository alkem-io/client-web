...
**Decision**: **Zero new i18n keys.** Every string this feature needs already exists and is already generic (not framing-specific wording) in all six locale files: `callout.addDocument` (doubles as both the add-affordance label and the add-dialog title — mirrors how `callout.addWhiteboard`/`callout.addMemo` are reused the same way), `callout.openDocument`, `callout.documentImportHint`/`documentImportMaxSize`/`documentImportOr`/`documentImportRemoveFile`, `callout.documentImportErrorUnsupported`/`documentImportErrorTooLarge`/`documentImportErrorMultiple`/`documentImportErrorFolder`/`documentImportErrorServiceUnavailable`, `contributionSettings.types.document`, and — confirmed generic, not per-`kind` — `deleteContribution.title`/`description`/`confirm` (no content-type-specific noun phrase exists or is needed there today; the existing copy is entity-agnostic: "Delete this contribution?" / "“{{title}}” will be deleted permanently."). Verified directly against `space.en.json` line-by-line, not assumed.

**Rationale**: The pre-scaffolded keys (confirmed by direct read of `space.en.json`) were placed for exactly this follow-up story; reusing them satisfies DRY and avoids duplicate-with-drift copy for the same concept (e.g. two different "upload a document" hint strings), and means this PR touches zero locale files — the cleanest possible outcome for constitution Architecture Std 3's key-parity requirement (nothing to keep in parity because nothing is added).

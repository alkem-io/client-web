...
- **FR-010**: On a server-side rejection reporting unsupported content or oversize, the add dialog MUST stay open and MUST render a human-readable inline message (reusing the existing `documentImportError*` message mapping used by the framing-upload flow) rather than a raw error code. A `STORAGE_SERVICE_UNAVAILABLE` rejection is a transport-level concern and MUST instead surface via the existing dialog-level toast (no auto-retry) — matching the framing-upload flow's already-established handling for the same error code (`CalloutFormConnector.tsx`). No rejection path MUST leave an orphaned contribution.

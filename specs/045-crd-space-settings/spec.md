...
- Q: Per-**column** "active phase" and "default post template" actions? → A: These are **per-column (innovation-flow step)** actions, NOT per-callout. Defer the visible menu CTA on each column card until the designer specifies where the column's overflow / kebab menu opens from. The mutations and hook wiring MUST be implemented now, hidden behind an internal `isDeferredMenuVisible` flag (default `false`) so the menu is ready to appear as soon as the designer's spec lands.
...
Each column card has an inline-editable **title** and **description** — the column represents the space's innovation-flow step / space-level tab. Below the column title/description, the callouts currently assigned to that space tab are listed with grab handles for drag-reorder (within a column) and drag-move (between columns). System / built-in entries in a column are pinned — shown with a lock indicator and not draggable.
...
**Per-column "Active phase" and "Default post template" actions** — these two capabilities are **per-column** (per innovation-flow step / space-level tab), not per-callout. In the current MUI implementation they live on the column header's overflow menu. In this iteration the column-level menu's UI entry point is NOT placed on the screen because the designer has not yet specified where it opens from. The underlying mutations, hooks, and dialogs MUST be implemented and kept behind an internal flag (`isDeferredMenuVisible`, default `false`) so the column menu can be surfaced in a follow-up patch simply by placing the trigger on each column header.
...
1. **Given** the admin opens the Layout tab, **When** the page renders, **Then** four columns (Home / Community / Subspaces / Knowledge) are shown with inline-editable titles and descriptions, and their assigned callouts listed below.
...
15. **Given** no trigger for the deferred per-**column** menu is placed, **When** the admin views any column header, **Then** no "Active phase" / "Default post template" UI is visible — but the internal hooks / mutations are fully wired and unit-tested per column, ready for the designer's CTA placement on the column header.
...
A space admin opens the Account tab to review the space's URL, license plan, usage, visibility status, host information, and to access support. The tab renders in the prototype's layout but preserves every piece of data and permission the current MUI Account page exposes.

**Sections**:

- **URL** — read-only monospace display of the space URL with a Copy button and a helper line explaining that changing the URL requires contacting Alkemio.
- **License** — card showing the current plan name + badge, included features, and usage (e.g., "Spaces used: 2 / 5") with a progress bar. Includes links to "Change License" (when supported by the host-provider link) and "More about Alkemio licenses".
- **Visibility Status** — badge (Active / other) with a "Request Change" button that routes via the support contact (no in-app status change).
- **Host Information** — avatar + host name + badges. There is NO "Change Host" CTA; instead, the support contact message below explains how to change host (matches current behavior).
- **Support** — muted-bordered card with the message "If you want to change any of these settings, please contact the Alkemio team here." and a "Contact Alkemio Support" button that opens the existing support link.
- **Delete this Account** (only when the current permission allows it — `useDeleteSpaceMutation` via `LicenseActionBlock` today) — destructive CTA with the existing confirmation flow.

Everything in this tab is either read-only display or routes through pre-existing mutations / external links. No Save / Reset bar is shown.

**Independent Test**: Open Account. Verify the URL with a working Copy button. Confirm the License card shows plan + entitlements + usage. Confirm the Visibility Status badge shows the correct state. Confirm the Host card shows host avatar and badges, with NO Change Host button. Click Contact Alkemio Support — the support link opens. If the permission exists, click Delete Space and confirm the flow.

**Acceptance Scenarios**:

1. **Given** the admin opens the Account tab, **When** it renders, **Then** URL, License, Visibility Status, Host Information, Support, and (if permitted) Delete sections are visible.
2. **Given** the admin clicks the URL Copy button, **When** the click lands, **Then** the URL is copied to the clipboard and the button toggles to a check icon briefly.
3. **Given** the License card renders, **When** it loads, **Then** plan name, included features, usage numbers, and a progress bar are visible.
4. **Given** the Host Information card renders, **When** it loads, **Then** NO "Change Host" button is shown — only the support-contact message routes host changes.
5. **Given** the admin clicks Contact Alkemio Support, **When** they click, **Then** the existing support link opens.
6. **Given** the admin has delete permission, **When** they confirm the delete flow, **Then** `deleteSpace` fires (identical to current MUI behavior).
...
- Deferred menu (Layout per-callout): no CTA is placed? → The UI shows no trigger; all mutations remain wired and covered by unit tests so surfacing the CTA later is trivial.
...
- **FR-010** (Layout — deferred column menu): The Layout tab MUST implement (but not yet surface) a **per-column** overflow menu with "Active phase" and "Default post template" actions. These are column-level (innovation-flow-step-level) concerns, not per-callout. The underlying mutations, hooks, and dialogs MUST be fully wired and covered by unit tests, gated behind an internal `isDeferredMenuVisible` flag (default `false`). No visible overflow affordance is placed on column headers in this iteration; the designer will specify placement in a follow-up.
...
- **FR-024** (Account): The Account tab MUST render URL, License (plan + entitlements + usage progress bar), Visibility Status, Host Information (no Change Host CTA — support-contact message instead), Support contact, and Delete Space (if permitted), matching every field the current MUI Account page shows today.
...
- Layout: the UI CTA for the deferred per-**column** menu ("Active phase", "Default post template") is NOT placed in this iteration — the menu logic is implemented but not visible until the designer specifies placement on the column header.
...
- **SC-009**: The deferred Layout per-**column** menu ("Active phase", "Default post template") can be surfaced in a follow-up patch by placing a CTA on the column header only (no additional mutation / hook / dialog work) — verified by a hidden-behind-flag unit test that exercises both actions end-to-end against each column.
...
- The designer will specify the placement of the deferred Layout per-**column** menu on each column header in a follow-up. Until then, the menu entries ("Active phase", "Default post template") are implemented and tested per column but not surfaced in the UI.

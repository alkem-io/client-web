...
## Phase 4b: User Story 3 — `/required` page CRD migration (Priority: P2)

**Goal**: When an anonymous user is redirected to `/required` (or visits it directly) with the CRD toggle on, the page renders the CRD-styled "Access Restricted — please sign in" page inside CRD layout chrome, with sign-in / return-as-guest actions wired through real `<a href>` anchors. Toggle off, the existing MUI `AuthRequiredPage` renders unchanged.
...
- [ ] T022 With CRD toggle OFF (`localStorage.setItem('alkemio-design-version', '1')`), reproduce all flows from `quickstart.md` and confirm zero visual / behavioral regression vs. production `develop` for: (a) authenticated user on private Space → MUI `Error403` inside `TopLevelLayout`; (b) anonymous user on private Space → silent redirect to `/required?returnUrl=...` → MUI `AuthRequiredPage` inside `TopLevelLayout`; (c) `NonAdminRedirect` → `/restricted` → MUI `Error403`; (d) direct visit to `/restricted` → MUI `Error403` + `logInfo('Attempted access to: ${origin}')`; (e) direct visit to `/required` → MUI `AuthRequiredPage` (US3 toggle-off check); (f) unrelated boundary error (e.g., a route that throws `NotFoundError`) → MUI `Error404`. Compare each render side-by-side with a fresh checkout of `develop` (or visit `develop` deployed elsewhere) to confirm pixel-level parity.

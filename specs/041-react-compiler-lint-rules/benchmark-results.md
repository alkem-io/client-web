...
**Run date**: 2026-07-20 · **re-measured 2026-07-29** (post-T026 migration — figures below reflect the re-run)
...
> metrics introduced with this work: **React Compiler coverage 1286/1286 (100%)**
> (`pnpm compiler:healthcheck`) and **INP 32 ms** (worst interaction, well under the 200 ms
> "good" threshold). Future performance comparisons should use that anchor, not the stale
> March dev baseline.

> **Read this first — what this PR actually changes.** The PR ships the ESLint
...
| Check | Result |
|---|---|
| `pnpm eslint .` | **0 errors, 0 warnings** (exit 0) |
| `pnpm lint` (typecheck:native + biome ci + eslint) | **pass** (exit 0) |
| Rule severity | `error` — a new `useMemo`/`useCallback`/`memo`/`React.memo` fails lint **and** the pre-commit hook |

The rule shipped at **error** level (per FR-002 — the warn→error transition completed once
every usage was annotated). After T026 there are **11** `no-restricted-syntax` exceptions
across 8 files (all genuine third-party/lifecycle), each carrying an
`eslint-disable-next-line … -- <reason>` comment. Adding a new
...
`pnpm build`: **succeeds, 0 errors, ~43s.**
...
|---|---|---|
| Home (`/`) | **95 / 100** | 1327 ms |
| Welcome Space (`/welcome-space`) | **94 / 100** | 1355 ms |
| Spaces (`/spaces`) | **94 / 100** | 1353 ms |

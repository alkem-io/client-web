...
**Independent Test**: Sign in (CRD on), click the floating button (≥ 640px viewport) → panel opens to the list with Guidance pinned, search filters the list, select a conversation → read its history → back to list. *(Amended 2026-08-21: the header messages icon is retained and opens the same panel; on phones it is the only entry.)*

### Implementation

- [X] T010 [P] [US1] Create `src/crd/components/chat/FloatingChatLauncher.tsx` (60×60 fixed bottom-right button, lucide `MessageCircle`, unread `Badge` overlay; props `unreadCount?`, `isOpen`, `hidden?`, `onClick`, `ariaLabel`) *(As shipped: `size-12` (48px), `bottom-20 right-4` with `lg:bottom-4`. Amended 2026-08-21: `hidden sm:flex` — the launcher does not render below 640px.)*
...
- [X] T016 [US1] Create `src/main/crdPages/unifiedChat/UnifiedChatLauncher.tsx` — mount `FloatingChatLauncher` + lazy `ChatPanel`/connector; hide rules (auth route + fullscreen editor; NOT mobile — *superseded 2026-08-21: the launcher is additionally hidden below 640px via `hidden sm:flex`; the header messages icon is the phone entry point*). **Source the launcher unread badge from the lightweight `useUnreadConversationsCount` (runs while the panel is closed); when the panel is open, the full `useUnifiedConversations.totalUnreadCount` is authoritative. Confirm guidance is included now that the filter is removed.** — depends on T010, T011, T015
...
- [X] T018 [US1] Remove the CRD Header messages icon + `onMessagesClick`/`unreadMessagesCount` wiring in `src/crd/layouts/Header.tsx` and `src/main/ui/layout/CrdLayoutWrapper.tsx` (combined unread now lives only on the launcher badge) *(Reversed: the header icon was ultimately kept and rewired to open the unified panel — and since 2026-08-21 it is the only chat entry on phone-width viewports, where the launcher is hidden.)*

**Checkpoint**: Floating button → panel → pinned-guidance list → search → select → read history → back. ~~Header icon gone.~~ *(Header icon retained — see T018 note.)* MVP demoable.
...
- [ ] T043 [P] Mobile responsiveness verification — requires a running app: `ChatPanel` full-screen expansion (via `useScreenSize`), list/thread/composer reachable on small screens *(open via the header messages icon — the launcher is hidden below 640px since 2026-08-21)*.

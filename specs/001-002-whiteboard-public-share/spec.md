...
3. **Given** a GraphQL error occurs when fetching whiteboard authorization, **When** the Share dialog renders, **Then** the UI shows a loading state or gracefully hides guest controls (no error flash)
...
- What happens when the GraphQL query for whiteboard `authorization.myPrivileges` times out or fails?
  - UI must show loading state or gracefully hide guest controls (no error flash to user)
...
- **FR-005**: Share dialog MUST handle missing or delayed `authorization.myPrivileges` data gracefully (show loading state, not errors)
...
- **FR-008**: Frontend MUST declare explicit TypeScript interfaces for component props (not export generated GraphQL types)
- **FR-009**: Frontend MUST handle GraphQL errors for whiteboard queries (timeout, network failure, malformed response)

**State Management Requirements:**

- **FR-010**: Authorization privileges MUST live in Apollo cache via existing whiteboard query structure (no client-side privilege calculation)
- **FR-011**: UI MUST react to Apollo cache updates when `myPrivileges` array changes (via subscription, refetch, or optimistic update)
- **FR-012**: Guest access toggle visibility MUST update within 2 seconds of privilege change in cache

**Accessibility & Performance Requirements:**

- **FR-013**: Guest access toggle MUST remain keyboard-navigable when visible
- **FR-014**: Guest access toggle MUST have proper ARIA labels for screen readers
- **FR-015**: Checking `myPrivileges` array MUST add <10ms overhead (simple array check, no GraphQL query needed)

### Key Entities

- **authorization.myPrivileges**: Array of privilege strings on whiteboard/callout GraphQL objects. Calculated by backend based on user roles, Space settings, and whiteboard ownership. Includes new `PUBLIC_SHARE` privilege when user is authorized to enable guest access.
...
- **WhiteboardShareControls Component**: React component in Share dialog that conditionally renders Guest access toggle based on presence of `PUBLIC_SHARE` in `authorization.myPrivileges` array.
...
- **SC-005**: GraphQL errors for whiteboard queries result in **graceful degradation** (loading state or hidden controls, no error flash)
...
- Guest access toggle visibility must be deterministic (based solely on `myPrivileges.includes('PUBLIC_SHARE')`)
- Frontend must never implement authorization logic (e.g., checking Space settings client-side)
- All authorization state must come from Apollo cache `myPrivileges` array (no local state)
...
1. **Privilege Enum Value**: Should the privilege be named `PUBLIC_SHARE`, `WHITEBOARD_PUBLIC_SHARE`, or `ENABLE_GUEST_ACCESS`?
   - **Recommendation**: `PUBLIC_SHARE` (concise, consistent with privilege naming patterns)

2. **Error Handling Strategy**: When whiteboard query fails, should UI hide controls or show "Unable to determine permissions" message?
   - **Recommendation**: Hide controls silently (safer default - prevents unauthorized actions)

3. **Cache Update Mechanism**: Should frontend use GraphQL subscriptions, polling, or refetch-on-focus for privilege updates?
   - **Recommendation**: Refetch on Share dialog open + Apollo cache updates (simpler than subscriptions)

4. **TypeScript Type Safety**: Should `myPrivileges` array be typed as `string[]` or use enum/union type for known privileges?
   - **Recommendation**: Use generated enum type from GraphQL schema (type-safe privilege checks)

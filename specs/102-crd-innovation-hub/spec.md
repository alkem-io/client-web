...
- **FR-017**: Field-length and format validation MUST match the validation rules already enforced on the legacy form (subdomain format, name length, markdown text length, tagline mid-text length) — no behavioural regression.
...
- **SC-003**: An anonymous visitor opening `/hub/<existing-slug>` on the new design sees the page render its banner, name, description, and at least the first row of Space cards within 1 second of HTML response on a standard broadband connection.

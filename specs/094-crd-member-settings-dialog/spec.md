...
- The CRD presentational confirmation prompt is implemented as a separate dialog primitive (or a CRD `AlertDialog`-style component if/when one is added). Until that primitive exists, the integration layer renders the confirmation as a second `Dialog` with destructive styling on the Confirm action — accepted as interim state.

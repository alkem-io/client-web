...
- Four clarifications were resolved in the 2026-06-09 session (see spec `## Clarifications`): prompt graph → full native CRD migration this round; Knowledge Base → promoted to a full-page CRD route, creation wizard → CRD dialog (the CRD equivalent of the legacy modal); add-to-community picker → reuse existing CRD patterns; parity bar → behavioral parity in CRD visual language (not pixel-identical to legacy MUI).

...
**Route & account context**: launch points currently pass an in-memory `account` object. A full-page route can't carry an object, so the target account id (and display name) are passed via route state / query param to the new route (e.g. `/create-virtual-contributor`). The page reads it and resolves the account. Add a builder to `urlBuilders.ts` (`buildCreateVirtualContributorUrl(accountId?, name?)`) — never template inline (FR-015).

**Launch-point change**: `DashboardWithoutMemberships`, `DashboardWithMemberships`, `CrdUserAccountTab`, `CrdOrgAccountTab` stop rendering `{virtualContributorWizard}` inline and instead `navigate(buildCreateVirtualContributorUrl(...))`.
...
**Sub-dialogs stay dialogs** (sticky-chrome rule): `VCWizardCancelDialog` (confirm discard), `VCExternalAIDialog` (+ nested `ComingSoon` request form), `VCTryContributorDialog` (demo callout + `useSubscribeOnVirtualContributorEvents`).

...
## D2. Creation wizard — full-page route, single route with internal step state

**Decision**: Build the wizard as **one full-page CRD route** holding the existing step state machine internally (no per-step sub-routes). Relocate the orchestration logic from `src/main/topLevelPages/myDashboard/newVirtualContributorWizard/useVirtualContributorWizard.tsx` into an integration hook `useVcCreationWizard.ts`, **keeping all GraphQL calls and the step machine intact** and swapping the `DialogWithGrid` shell for a CRD full-page shell (`VCCreationWizardView`). Steps `initial → {addKnowledge | existingKnowledge | externalProvider} → chooseCommunity → tryVcInfo` (+ `loadingStep`) become page sections rendered by a step switch.

**Route & account context (as implemented)**: launch points pass an in-memory `account` object. A full-page route can't carry an object, so the account is carried via **React Router location state** — `navigate(buildCreateVirtualContributorUrl(), { state: { account, accountName } })` — *not* a query param (the earlier query-param idea was dropped: state avoids any new account-by-id query/codegen and preserves the org-account path). The page reads `location.state?.account`; when absent (a direct URL hit), it falls back to the current user's account via `useNewVirtualContributorMySpacesQuery`.

The wizard is **nested under the owning entity's settings** (route segment `create-virtual-contributor`), because a VC can be created for a **user** (`/user/<slug>/settings/create-virtual-contributor` — incl. `/user/me/...` and **another user's account** viewed by a platform admin) **or an organization** (`/organization/<slug>/settings/create-virtual-contributor`). It is mounted as a **sibling of the settings-tab shell** route in **both** route trees — `CrdUserSettingsRoutes.tsx` (serves any `/user/:userNameId/settings/*`) and `CrdOrgSettingsRoutes.tsx` — so it renders full-page (no settings tab nav) while inheriting the CRD shell (`CrdLayoutWrapper`) + auth (and, for org, the `OrganizationProvider`) from the surrounding route tree. It is therefore **not** a top-level route.

The builder `buildCreateVirtualContributorUrl(entityProfileUrl?)` in `urlBuilders.ts` takes the entity's `profile.url` (e.g. `/user/me`, `/user/<slug>`, `/organization/<slug>`; defaults to `/user/me` for the dashboards) and returns `<profileUrl>/settings/create-virtual-contributor` with **no query params** (the account is carried via location state; an earlier `?accountId/?name` hint was removed as unused) — never template inline (FR-015).

The page is split into a **shared body** (`CrdVCCreationWizard` — reads account from location state, runs the hook, renders the view) plus **two thin per-context wrappers** that own the breadcrumb trail: `CrdVCCreationWizardPage` (user — `useUserPageRouteContext`, `User` icon) and `CrdOrgVCCreationWizardPage` (org — `useOrganizationContext`, `Building2` icon). Each mirrors that entity's account-tab trail (e.g. *My user › Settings › Account › Create Virtual Contributor*).

**Launch-point change**: `DashboardWithoutMemberships`, `DashboardWithMemberships`, `CrdUserAccountTab`, `CrdOrgAccountTab` stop rendering `{virtualContributorWizard}` inline and instead `navigate(buildCreateVirtualContributorUrl(entityProfileUrl), { state: { account, accountName } })` — dashboards pass nothing (→ `/user/me`), the user account tab passes `useUserPageRouteContext().profileUrl`, the org account tab passes `organization.profile.url`.
...
**Sub-dialogs stay dialogs** (sticky-chrome rule): `VCExternalAIDialog` (+ nested `ComingSoon` request form). *(As implemented: the cancel-confirm dialog was dropped — the full page has no close button, so the user exits via the breadcrumb trail and in-progress input is discarded on navigation. The interactive try-VC dialog is out of scope — a space-dashboard surface.)*

**Rationale**: Single-route + internal state is the minimal faithful port of the existing state machine, preserves all data wiring, and satisfies the "full-page" clarification. Sub-routes would multiply routing/guard code for no user benefit.

**Alternatives considered**: (a) Keep it a CRD dialog — rejected per clarification. (b) Per-step sub-routes — rejected (more code, deep-link-into-mid-wizard semantics undesirable). (c) Rewrite the state machine — rejected (risk; the logic is reused verbatim).

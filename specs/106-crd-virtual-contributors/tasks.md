...
- [X] T015a [US1] (FR-025) Add a paperclip file-attach affordance to each Document row in the add-knowledge step: extend `VCCreationWizardView` props with `onDocumentUpload?`/`documentUploadAccept?`, render `RowAttachFileButton` per document row (writes the uploaded URL into the row's URL field, defaults the name to the filename when empty) in `src/crd/components/virtualContributor/creationWizard/VCCreationWizardView.tsx`, and wire it in `CrdVCCreationWizardDialog.tsx` via `useReferenceFileUpload(useStorageConfigContext())` (account bucket, `temporaryLocation: true`; hidden when the viewer lacks upload privilege). Add `wizard.addKnowledge.attachDocument` to all six `contributorSettings.<lang>.json`.

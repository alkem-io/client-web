...
6. **Given** the user is on the add-knowledge step adding a Document, **When** they click the row's paperclip and pick a file, **Then** the file is uploaded to the owning account's storage bucket as a temporary file, the row's URL is populated with the resulting link (and the name defaults to the filename if empty), and on VC creation the file is associated with the new VC.
...
- **FR-025**: In the creation wizard's add-knowledge step, each **Document** row MUST offer a direct file-attach affordance (paperclip), parity with the CRD References field: picking a file uploads it and writes the resulting URL into the row's URL field (defaulting the row name to the filename when empty). Because the VC's own storage bucket does not exist yet at creation time, the upload MUST target the owning **account's storage bucket** as a **temporary** file (`temporaryLocation: true`), reusing the same storage scope the wizard already uses for body-of-knowledge markdown image uploads; the server relocates the file to the VC on creation. The affordance MUST reuse the shared CRD attach-file building blocks (`RowAttachFileButton` + the `useReferenceFileUpload` connector hook) and MUST be hidden when the viewer lacks upload privilege on the account bucket (link-only fallback), consistent with the markdown image-upload gating.

...
- **Architecture Standards #3 (new strings in CRD per-feature namespaces)** — PASS/N/A. No new i18n keys: the (future) preview image's alt text reuses the *existing* `typeLabel` string (`callout.documentText`/`documentSpreadsheet`/`documentPresentation`, already in `crd-space`, all six locales) rather than adding new keys — see Research R4.

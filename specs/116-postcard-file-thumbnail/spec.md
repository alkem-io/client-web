...
- **A-003**: The `CalloutContributionType.COLLABORA_DOCUMENT` case in the contributions grid (`ContributionsPreviewConnector`'s `ContributionCard` switch, which currently has no case for it and silently renders nothing) is a distinct, pre-existing gap not referenced by #9575's checklist or by #1842's checklist line (both are specifically about the callout **framing** preview, i.e. `PostCard`/`CalloutDetailDialog`). Fixing it is out of scope for this story to avoid scope creep beyond what was asked; it is a reasonable candidate for a follow-up story if OfficeDocs contributions (plural, per-response) are reactivated as a `CalloutContributionType` in a future phase.
- **A-004**: No new shared `src/crd/lib/` utility is extracted for the type→visual mapping. `CalloutCollaboraPreview` is currently the *only* consumer of `CollaboraDocumentPreviewType` in the UI layer, so centralizing the mapping inside that one component (FR-002) satisfies "one mechanism, not a parallel one" for this story's actual scope without speculative extraction (CLAUDE.md: "avoid over-engineering — only make changes directly requested or clearly necessary"). If/when a second consumer needs the same type→visual mapping (e.g. a future generic file listing from alkemio#1673), extracting it to `src/crd/lib/` is the natural, low-risk follow-up — the mapping's shape (a `Record` keyed by the type) is already structured to make that extraction mechanical.

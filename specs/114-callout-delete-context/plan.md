...
Turn the generic callout delete confirmation into a context-aware dialog that shows what will be permanently removed. The presentational surface (`ConfirmationDialog` → `DeleteCalloutDialog`) gains a variable-height body: a **table** headed by the scope sentence — "The whiteboard and 20 contributions will be deleted" (rich framing body composed with the exact contribution total) — with a row per listed contribution (first 3: bold title + one-line markdown description preview), a comment-count row ("27 comments will be deleted"), and a final clip-icon row ("including attached files and links"); named links list below. The confirm button reflects the scope ("Delete callout and all contents" vs. "Delete"). Per the 2026-07-02 clarification (**Option A**), the summary is built **only from data that arrives with the callout's standard load**: the `CalloutDetails` fragment's `contributions` selection is extended with title + description stubs (`post`/`whiteboard`/`memo`/`collaboraDocument` → `profile { displayName description }`; `link` already selected) — **one fragment extension + `pnpm codegen`, no new query** — so opening the dialog issues zero extra network requests. A pure mapper in the integration layer converts `CalloutDetailsModelExtended` → a plain-TS summary; the CRD component renders it with i18n copy from the `crd-space` namespace (all six languages, key parity).
...
**Storage**: Apollo normalized cache — **no new persistence, no new query**; one additive `CalloutDetails` fragment extension (contribution title + description stubs) + `pnpm codegen` in the same PR (Option A: data rides the standard load)  
...
**Scale/Scope**: One additive fragment extension (+codegen), one TS model widening, one CRD prop extension, one enhanced CRD dialog + one small summary sub-component, one pure mapper, i18n keys in six files, unit tests. No routing, no server schema change, no new dependency.
...
| III. GraphQL Contract Fidelity | **No server schema change; no new query.** One additive extension of the `CalloutDetails` fragment (title + description stubs on `contributions`), regenerated via `pnpm codegen` in the same PR with the generated artifacts committed. `CalloutDetailsModel.contributions` widens to match (structural mapping in `useCalloutDetails` — no mapping-site change). No generated types leak into CRD props. | ✅ Pass |
...
| V. Experience Quality & Safeguards | Contribution summary uses a semantic `<table>` (header row + `<th scope="row">` titles) with the links as a semantic `<ul>`/`<li>` list; decorative icons `aria-hidden`; destructive styling + keyboard/AT reachable; i18n parity across six languages with plural forms; unit tests for the pure mapper + summary render. | ✅ Pass |
...
├── calloutsSet/useCalloutsSet/CalloutsSetQueries.graphql   # EXTEND: `CalloutDetails` contributions gain title + description stubs (post/whiteboard/memo/collaboraDocument → profile { id displayName description })
└── callout/models/CalloutDetailsModel.ts                   # EXTEND: contributions item type widens to carry the titled stubs

src/core/apollo/generated/                                  # REGENERATED: `pnpm codegen` output committed
...
├── CalloutDeletionSummary.tsx      # NEW: presentational summary table (scope header, title + description-preview rows, comments row, clip attachments row) + links list
└── calloutDeletionSummary.types.ts # NEW: plain-TS view-model types (CalloutDeletionSummaryModel etc.) shared by mapper + CRD

src/crd/i18n/space/
├── space.en.json                   # EXTEND: deleteCallout.* (confirmAll, table-header variants, comments row, contentType terms, attachments note, moreLinks; description reworded neutral — no contributions/comments claim)

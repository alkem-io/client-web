...
- [X] T001 Register the new `crd-subspace` i18n namespace in `src/core/i18n/config.ts` by adding it to `crdNamespaceImports` (lazy-load all 6 languages from `src/crd/i18n/subspace/subspace.<lang>.json`) — mirror the pattern used for `crd-space`
- [X] T002 Add `'crd-subspace'` to the i18next namespace type union in `@types/i18next.d.ts` (or wherever the existing CRD namespace types are declared) so `useTranslation('crd-subspace')` is type-checked
...
- [~] T004 ~~Add new GraphQL document at `src/domain/space/community/CommunityMemberCount.graphql`~~ — **DROPPED**. The "true total member count" feature is deferred. The banner avatar stack now shows lead-user avatars only (no `+N` overflow chip). If the feature is revived, a separate spec should add the GraphQL document.
- [X] T005 Run `pnpm codegen` (requires backend on `localhost:4000`) and commit the regenerated files in `src/core/apollo/generated/` (per Constitution Engineering Workflow #2)
- [X] T006 [P] Add `buildSubspaceSettingsUrl(subspaceUrl: string): string` to `src/main/routing/urlBuilders.ts` returning `${subspaceUrl}/settings` (single migration point per plan D11)

**Checkpoint**: i18n namespace registered, settings URL helper available — Foundational phase can begin.
...
- [ ] T014 [US1] Create `src/main/crdPages/subspace/hooks/useCrdSubspace.ts`. Orchestrates: `useSubspaceContext()` (id, level, about, permissions, parentSpaceId), `useSpaceAboutDetailsQuery({ spaceId: parentSpaceId, skip: !parentSpaceId })` for the parent banner (per research R3), `useApplicationButton({ spaceId, parentSpaceId })` for the join/apply CTA (per research R8). Returns `CrdSubspacePageData`. Depends on T012. **Note**: the original plan included a `useCommunityMemberCountQuery` for the true total member count — that feature is deferred (see T004). The banner shows lead avatars only.
...
- [X] T016 [P] [US1] Create `src/main/crdPages/subspace/dialogs/CrdSubspaceCommunityDialogConnector.tsx`. Props: `{ open, onOpenChange, spaceId, roleSetId }`. Internally calls `useRoleSetManager(roleSetId)`, maps results to `MemberCardData[]` (use the existing community data mapper at `src/main/crdPages/space/dataMappers/communityDataMapper.ts`), renders `<SubspaceCommunityDialog>{<SpaceMembers members={...} pageSize={9} />}</SubspaceCommunityDialog>`. **This connector is also reused by L0** — see T030. **POLISH REFACTOR (plan D6)**: the connector was relocated to a layer-neutral path so the L0 layout could consume it without crossing the L0→L1 directory boundary. Final location: `src/main/crdPages/space/dialogs/CrdSpaceCommunityDialogConnector.tsx`; the function was renamed to `CrdSpaceCommunityDialogConnector`. Both `CrdSpacePageLayout` and `CrdSubspacePageLayout` import from the new path.
...
- [ ] T021 [US1] Create `src/main/crdPages/subspace/layout/CrdSubspacePageLayout.tsx`. Mirrors `CrdSpacePageLayout` for L1: calls `useCrdSubspace()`, renders `<SubspaceHeader />` (with `onMemberClick` opening the Community dialog), `<SubspaceSidebar />` (right column, fixed), and `<Outlet />` for tab content. Manages dialog open/close state for the four Quick Action dialogs (`Community / Events / Activity / Index / Subspaces`) and the About dialog. Calls `useSetBreadcrumbs([{ label: parentSpaceName, href: parentSpaceUrl }, { label: subspaceName }])` (per FR-006). *(Later extended: since 2026-08-21 each space/subspace hop also carries `avatar: { src, initials }` — L0 hops use the cardBanner, L1/L2 hops their avatar visual — rendered as a 20px rounded square; see 045-crd-space-settings/data-model.md "Subspace breadcrumbs".)* Depends on T007–T010, T014, T016–T020
...
- [~] T029 [US3] ~~Modify `src/main/crdPages/space/dataMappers/spacePageDataMapper.ts`: extend `mapMemberAvatars` to return `BannerCommunityData = { avatars: MemberAvatar[]; totalCount: number }`~~ — **DROPPED** with T004. `mapMemberAvatars` keeps its existing `MemberAvatar[]` shape; no `totalCount` field, no `+N` overflow chip rendered.
- [ ] T030 [US3] Modify `src/main/crdPages/space/layout/CrdSpacePageLayout.tsx`: pass `onMemberClick={() => setCommunityDialogOpen(true)}` to `<SpaceHeader />` and render `<CrdSubspaceCommunityDialogConnector open={communityDialogOpen} onOpenChange={setCommunityDialogOpen} spaceId={spaceId} roleSetId={roleSetId} />` (the same connector built in T016). The `!isLevelZero` bailout STAYS unchanged. Per FR-027 / plan D6. Depends on T016. **Note**: the original plan also passed a `memberCount` derived from `useCommunityMemberCountQuery` — dropped with T004; the avatar stack shows lead avatars only.
- [ ] T031 [US3] Manual verification of US3 per quickstart §5: open an L0 space and click the avatar stack — confirm the community dialog opens. Repeat with a zero-lead space to confirm FR-028 (avatar stack hidden, no empty group rendered)
...
- [X] T034 [P] [US5] Create `src/crd/i18n/subspace/subspace.nl.json` translating every key from `subspace.en.json` — pulled forward into Phase 1 to unblock TS module resolution
- [X] T035 [P] [US5] Create `src/crd/i18n/subspace/subspace.es.json` translating every key from `subspace.en.json` — pulled forward into Phase 1
- [X] T036 [P] [US5] Create `src/crd/i18n/subspace/subspace.bg.json` translating every key from `subspace.en.json` — pulled forward into Phase 1
- [X] T037 [P] [US5] Create `src/crd/i18n/subspace/subspace.de.json` translating every key from `subspace.en.json` — pulled forward into Phase 1
- [X] T038 [P] [US5] Create `src/crd/i18n/subspace/subspace.fr.json` translating every key from `subspace.en.json` — pulled forward into Phase 1
...
- [~] T042 [P] ~~Add a unit test for `mapBannerCommunity` returning `{ avatars, totalCount }`~~ — **DROPPED** with T004 / T029. `mapMemberAvatars` keeps its original `MemberAvatar[]` return shape; no new test required.
...
### Polish-phase refinements (post-MVP, completed)

These additions / refactors were made after the MVP shipped during code-review and design-refinement passes. They are documented here for traceability; the corresponding decisions live in plan D6, D14, D15, D16.

- [X] T050 Create `src/crd/components/space/sidebar/InfoBlock.tsx` (plan D14). Shared blue-panel widget consumed by both `SpaceSidebar` (L0) and `SubspaceSidebar` (L1). Renders `description` through `MarkdownContent` (with overrides forcing pure white text, `text-body` size, and headings downgraded to body), an inline lead list (with white-on-blue `border-t border-white/15` separator), and an optional hover edit pencil + whole-block clickable overlay. Exports the shared `LeadItem` type. Replaces the previously-separate L1 info card and the L0 community-variant `<LeadBlock>` widget.
- [X] T051 Delete `src/crd/components/space/sidebar/LeadBlock.tsx` (orphaned after T050). Move all consumers to import `LeadItem` from `InfoBlock.tsx`.
- [X] T052 Refactor `src/crd/components/space/SubspaceSidebar.tsx` to render `<InfoBlock description leads onEditClick={onAboutClick} />` at the top, drop its own info-card markup, drop the standalone "About this Subspace" outline button, drop `whyMarkdown` / `tagline` from `SubspaceSidebarData`, add `description: string`. Quick Actions and Virtual Contributor sections unchanged.
- [X] T053 Refactor `src/crd/components/space/SpaceSidebar.tsx` to render the shared `<InfoBlock>` at the top of every variant (with `leads` prop), drop the standalone "About this Space" outline button (clicking the InfoBlock pencil replaces it), drop the standalone `<LeadBlock>` from the community variant.
- [X] T054 Update `src/main/crdPages/subspace/dataMappers/subspacePageDataMapper.ts` and `src/main/crdPages/subspace/hooks/useCrdSubspace.ts` to source the sidebar description from `subspace.about.profile.description` instead of `subspace.about.why` + `subspaceProfile.tagline`. Map leads to the shared `LeadItem[]` shape with `type: 'person'`.
- [X] T055 Add `mapSidebarLeads(leadUsers, leadOrganizations?)` to `src/main/crdPages/space/dataMappers/spacePageDataMapper.ts` that produces `LeadItem[]` from either source (users default to `type: 'person'`, orgs to `type: 'org'`).
- [X] T056 Create `src/main/crdPages/space/hooks/useCrdSpaceLeads.ts` (plan D15). Calls `useSpaceAboutDetailsQuery` and returns `LeadItem[]`. Used by the L0 Dashboard, Subspaces, and Knowledge tabs to feed leads to `<SpaceSidebar />` because `SpaceContext`'s `SpaceAboutLight` fragment does not include `leadUsers` / `leadOrganizations`. The Community tab continues to compute leads via `useCrdSpaceCommunity` (Apollo dedupes regardless).
- [X] T057 Update L0 tab pages (`CrdSpaceDashboardPage.tsx`, `CrdSpaceSubspacesPage.tsx`, `CrdSpaceCustomTabPage.tsx`, `CrdSpaceCommunityPage.tsx`) to: (a) source description from `space.about.profile.description` (drop `tabDescription` fallback for the sidebar widget), (b) pass `leads={sidebarLeads}` to `<SpaceSidebar />`, (c) wire `onAboutClick` to navigate to `${space.about.profile.url}/settings/about`.
- [X] T058 Create `src/crd/components/space/TabStateHeader.tsx` (plan D16). Shared description-on-left + action-on-right strip used by every L0 tab page for the per-tab Add Post / Invite Member / Create Subspace / Create Subspace buttons (all `size="sm"`).
- [X] T059 Move community dialog connector from `src/main/crdPages/subspace/dialogs/CrdSubspaceCommunityDialogConnector.tsx` to `src/main/crdPages/space/dialogs/CrdSpaceCommunityDialogConnector.tsx` (plan D6, see T016 note above). Both `CrdSpacePageLayout` and `CrdSubspacePageLayout` updated to import from the new location.
- [X] T060 Remove the `useCallback` wrapper in `src/main/crdPages/subspace/hooks/useCrdSubspaceFlow.ts` (React Compiler compliance — the project disallows manual memoization).
- [X] T061 Wire L0 banner avatar-stack click to the shared community dialog (`onMemberClick` on `<SpaceHeader>`). Mounts `<CrdSpaceCommunityDialogConnector>` after `<SpaceShell>`. Closes the US3 acceptance gap left from the original implementation.
- [X] T062 Collapse the L1 layout's dual community-dialog state (`activeDialog` + `communityFromBanner`) to a single `activeDialog` state machine.
- [X] T063 Fix `src/crd/lib/getInitials.ts` to iterate by Unicode code point (`Array.from(word)[0]`) so non-Latin scripts (Cyrillic, CJK, surrogate pairs) render correctly instead of `?`.
- [X] T064 Split sidebar handlers into pencil and About-button (plan D17). `SpaceSidebar` and `SubspaceSidebar` each now take **two** distinct callbacks: `onEditClick` (InfoBlock pencil → `${profileUrl}/settings/about` — both L0 and L1) and `onAboutClick` (outline About button → read-only `SpaceAboutDialog`). The InfoBlock continues to take a single `onEditClick`.
- [X] T065 Restore the "About this Subspace" outline button below the InfoBlock in `SubspaceSidebar` (with `Info` icon + `t('sidebar.about')` from `crd-subspace`); restore the equivalent "About this Space" button in `SpaceSidebar` for the **home variant only** (with `Info` icon + `t('sidebar.aboutSpace')` from `crd-space`). The other L0 tab variants (Community / Subspaces / Knowledge) continue to render the InfoBlock without a button below it. (Plan D17.)
- [X] T066 Create `src/main/crdPages/space/dialogs/CrdSpaceAboutDialogConnector.tsx` (plan D17). Mirrors `CrdSubspaceAboutDialogConnector` but reads via `useSpace()` + `useSpaceAboutDetailsQuery` for the level-zero space. Mounted by `CrdSpaceDashboardPage` (the only L0 variant that renders the About button).
- [X] T067 Re-wire layouts/tab pages for the split (plan D17). `CrdSubspacePageLayout`: pencil → `navigate('${subspaceUrl}/settings/about')`, button → `setAboutOpen(true)`. `CrdSpaceDashboardPage`: same shape, with new local `aboutOpen` state and a mounted `<CrdSpaceAboutDialogConnector>`. `CrdSpaceCommunityPage` / `CrdSpaceSubspacesPage` / `CrdSpaceCustomTabPage`: rename existing `onAboutClick` prop → `onEditClick` (no About button rendered on those variants).

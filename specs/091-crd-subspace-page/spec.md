...
- **FR-015**: The sidebar MUST contain a primary info card showing the subspace's purpose ("why" / tagline) and lead user(s).
- **FR-016**: The info card MUST NOT contain a "Challenge Statement" header label or icon.
- **FR-017**: The system MUST render an "About this Subspace" button **outside and below** the info card. Clicking it MUST open the existing platform-wide About dialog populated with the subspace's full public details.

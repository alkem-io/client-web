...
- **FR-017**: The info block MUST expose an **edit-on-hover** affordance: a `<Pencil>` icon revealed on hover/focus in the top-right corner, with the entire panel clickable. Clicking the panel MUST navigate to `${spaceProfileUrl}/settings/about` (the edit surface) on **both** L0 and L1. A separate **"About this Space" / "About this Subspace"** outline button below the info block opens the read-only About dialog populated with the relevant entity's full public details (the existing `SpaceAboutDialog`). The button is rendered on the L0 home tab and on every L1 / L2 subspace sidebar; it is omitted from the other L0 tab variants (Community, Subspaces, Knowledge) to avoid redundancy with their own primary actions. Pencil = edit, button = view — the two paths never share a handler.
...
- **Mobile responsiveness**: The right sidebar collapses to a bottom-of-page section on small screens, mirroring the L0 mobile pattern. *(Amended 2026-08-21: sidebar content is reached through the mobile menu drawer, opened by the hamburger at the **left** end of the fixed bottom bar (`SubspaceFlowTabs`) or by a left-edge swipe gesture (`useEdgeSwipe`, below `lg`, non-settings pages) — there is no "More" affordance.)*

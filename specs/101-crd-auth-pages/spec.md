...
- **FR-025**: Once this spec ships, the MUI auth page components (`LoginPage`, `RegistrationPage`, `SignUp`, `RecoveryPage`, `VerificationPage`, `EmailVerificationRequiredPage`, and the MUI auth-error route) are no longer routed to. They MUST remain in the codebase as orphaned dead code and be removed in a dedicated follow-up cleanup PR once the CRD auth flow is validated in production — not in this spec. The shared Kratos MUI component layer (`KratosUI`, `KratosForm`, `KratosInput`, etc.) MUST stay regardless, because the still-MUI `/identity/settings` flow depends on it.

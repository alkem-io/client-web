...
- [X] T019 [US1] Implement `UserPublicProfileView` at `src/crd/components/user/UserPublicProfileView.tsx` per `UserPublicProfileViewProps` in `contracts/publicProfile.ts`. Composes the hero + sidebar + tab strip + sections per the shared visual layout (FR-040 / FR-041): outer `grid grid-cols-1 lg:grid-cols-12 gap-6`, sidebar `lg:col-span-3 lg:sticky lg:top-24 self-start`, right column `lg:col-span-9`, both wrapped in the CRD `PageLayout` shell. On viewports below `lg`, the sidebar stacks **above** the right column in a single-column layout (it is **not** hidden — sidebar content is real, not chrome). Hero schema per FR-041 — avatar `w-28 h-28 md:w-32 md:h-32`, padding `pt-8 pb-6`, name uses the `text-profile-title` semantic token. **Renders Skeleton placeholders per region**, driven by the per-region `loading` shape (Q3 — `loading.hero`, `loading.organizations`, `loading.hostedResources`, `loading.memberships`); each region paints in independently as its driving query resolves (FR-009 / FR-042). Layout MUST NOT shift when a region transitions from skeleton to real content.

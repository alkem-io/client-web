...
- The User profile prototype reference (`prototype/src/app/pages/UserProfilePage.tsx`) is the only prototype available; there is **no prototype** for Organization or VC profiles. Those two pages adopt the User profile's CRD design language (avatar + identity block, sidebar + main column, card-based sections) and stay parity-only with current MUI for content and data.

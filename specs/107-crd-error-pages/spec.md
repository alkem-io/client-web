...
- **Q (Edge case): How is a 404 on a reserved MUI-only top-level path handled when CRD is enabled?**
  **A:** It stays on the MUI fallback, because `isCrdRoute` returns false for reserved top-level segments. No CRD chrome is forced onto an otherwise-MUI route.
  **Rationale:** Reuses the established `isCrdRoute` contract (FR-011); consistent with the existing 403 dispatch behaviour.
...
- **404 on a reserved top-level path that is itself MUI-only**: A reserved MUI path that 404s while CRD is enabled stays on the MUI fallback (consistent with `isCrdRoute` returning false for reserved segments) — no CRD chrome is forced onto an otherwise-MUI route.

...
| Arch Std #3 (i18n) | PASS (with a noted tradeoff) | New keys land in `crd-common` (the default, eagerly-loaded namespace) across all six locales in this PR; the core EN file is untouched. Std #3 says "per-feature namespaces", but this copy is shared verbatim by four surfaces spanning admin, organization and space features — placing it per-feature would duplicate it four times and strain Std #6f (DRY). `crd-common` already exists as the project's home for shared CRD copy. Flag for reviewer confirmation. |

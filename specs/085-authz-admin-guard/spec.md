...
**Independent Test**: Log in as a user without `GRANT` privilege on the target entity and navigate to its authorization admin page. Verify that the "Add user" control is visibly disabled, cannot be activated by click or keyboard, and shows an explanatory tooltip on hover/focus stating in plain language that permission is missing.
...
- **FR-002**: When the current user does not hold `GRANT` on the entity, every role-assignment control on the page — both the "Add" control and the per-member "Remove" control, for all contributor types — MUST be rendered in a disabled state that cannot be activated by mouse, keyboard, or assistive technology.
...
- **FR-006**: When any add-user or role-assignment request to the backend fails for any reason (authorization, validation, network, server), the UI MUST present a visible, dismissible error notification to the user; silent failure is not permitted. This notification MUST be produced at the mutation call site (the hook owning the assignment mutation), not by altering the platform's global error-handling pipeline.
...
- **FR-009**: When a user holds the `GRANT` privilege, the existing enabled behavior of the role-assignment controls and their flows MUST be preserved with no regression.
...
- **Role assignment action**: A user-initiated operation that changes role membership on the entity — assigning a contributor to a role or removing one from it, for any contributor type (user, organization, virtual contributor, platform role). Requires `GRANT` on the entity to succeed on the backend.
...
- Controls that require privileges other than `GRANT` (e.g., `UPDATE`, `DELETE`) are out of scope for this change unless they also currently silently fail — if so, the same pattern (disable + tooltip + error fallback) applies using the correct privilege for each.

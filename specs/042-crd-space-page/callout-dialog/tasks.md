...
- [X] **T023** Create `src/crd/forms/callout/LinksPrePopulateRows.tsx` (used inside `LinksPanel`): rows of title/url/description + add / delete + per-row paperclip file-attach (shared `RowAttachFileButton` + `onFileUpload`/`uploadAccept` props wired from the connector's `useReferenceFileUpload`; an uploaded file's URL fills the row url and the filename fills an empty title — FR-31, US8). Validates non-empty uri requires non-empty title (FR-85).
- [X] **T024** ~~Create `src/crd/forms/callout/ReferencesEditor.tsx`~~ **Superseded by T024c.** (Originally: a callout-specific title/url editor inside "More options" with add/delete, paperclip, FR-84 validation.)
- [X] **T024b** Extract the per-row paperclip into `src/crd/forms/references/RowAttachFileButton.tsx` and reuse it from the shared `ReferencesEditor` and `LinksPrePopulateRows` (single component per spec 045's consolidation decision; no behaviour change).
- [X] **T024c** (2026-05-21) **Unify the two `ReferencesEditor` variants into one.** Delete `src/crd/forms/callout/ReferencesEditor.tsx`; route `CalloutFormConnector`, `CalloutTemplateForm`, and `CrdPostContributionDialog` through the shared `src/crd/forms/references/ReferencesEditor` (canonical `{ id?, name, uri, description? }`, `crd-common` ns). The callout & post form models (`ReferenceRow` / `PostReferenceRow`) were renamed `title→name`, `url→uri` (description now optional) to match the GraphQL `Reference` type and the canonical row everything else already uses — so the three callsites pass rows straight through (no adapter). Validation, mappers and the post yup schema were updated to the new field names with no behaviour change. `useCrdCalloutForm` exposes a small `referenceRowErrors` helper that strips the `referenceRows.` prefix into the editor's `<i>.name`/`<i>.uri` contract (mirroring `InnovationPackForm`/`CommunityGuidelinesTemplateForm`). `LinkRow` (pre-populate links) keeps `title`/`url` — a Link is a `displayName`+`uri` contribution, not a `Reference`. Port the callout editor's `aria-describedby` wiring into the shared editor so callout a11y doesn't regress. Remove the now-orphaned top-level `references.*` block from the `crd-space` namespace (6 langs). The label text is identical between the two namespaces, so no user-visible change.
...
- [X] **T030 [P]** Translation keys: `contributionSettings.*` (incl. `contributionSettings.prePopulate.attachFile` for the Links & Files paperclip), `responseDefaults.*`, `references.*`, `forms.tagsHelpText`, `forms.notifyMembers`, `forms.allowComments` (label + description for T024a's framing-level toggle), `callout.document` across 6 languages.

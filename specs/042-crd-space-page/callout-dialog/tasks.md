...
- [X] **T024** Create `src/crd/forms/callout/ReferencesEditor.tsx` (inside "More options"): rows of title/url/description, add/delete, per-row paperclip file-attach (shared `RowAttachFileButton`), FR-84 validation.
- [X] **T024b** Extract the per-row paperclip into `src/crd/forms/references/RowAttachFileButton.tsx` and reuse it from both `ReferencesEditor` variants and `LinksPrePopulateRows` (single component per spec 045's consolidation decision; no behaviour change).

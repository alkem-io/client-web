...
| ~~`forms/callout/ReferencesEditor.tsx`~~ | **Removed (unified 2026-05-21).** The callout References editor is now the single shared `forms/references/ReferencesEditor` (canonical `{ id?, name, uri, description? }` rows). The callout & post form models (`ReferenceRow` / `PostReferenceRow`) were renamed `title→name`, `url→uri` to match the GraphQL `Reference` type and the canonical row everything else uses, so the three callsites (`CalloutFormConnector`, `CalloutTemplateForm`, `CrdPostContributionDialog`) pass their rows straight through — no adapter. No behaviour change. |
| `forms/references/RowAttachFileButton.tsx` | Shared per-row paperclip button (hidden file input + spinner during upload). Reused by the shared `ReferencesEditor` and `LinksPrePopulateRows`. Props: `accept?`, `disabled?`, `uploading`, `ariaLabel`, `onFile`. |

...
- **Create**: on a callout configured for memo contributions, members see an "Add memo" card in the contribution grid trailing slot. Clicking it opens a small dialog asking for a display name, calls `useCreateMemoOnCalloutMutation`, and immediately opens the newly-created memo in `CrdMemoDialog` for real-time editing. (MUI navigates to a memo URL after create; CRD opens the dialog in place — consistent with how the whiteboard add flow works today in CRD.) The create dialog's title input MUST seed from `callout.contributionDefaults.defaultDisplayName` (falling back to the i18n `callout.defaultMemoName` only when the callout has no default set); the new memo's initial `markdown` body MUST seed from `callout.contributionDefaults.postDescription` (defaults are shared with Post contributions per FR-43 — same two fields, same wiring). This mirrors the Post create flow (`PostContributionAddConnector` already does this).

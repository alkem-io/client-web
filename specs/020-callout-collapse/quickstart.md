...
- **Codegen deferred**: Until `pnpm codegen` runs against the server, generated inline types in `graphql-schema.ts` must be manually patched to include `layout` on `SpaceSettingsQuery`, `SpaceSettingsFragment`, and `UpdateSpaceSettingsMutation`.

...
- [ ] T002 [P] Extend `src/main/crdPages/useCrdEnabled.ts` to export `readCrdEnabledFromStorage(): boolean | null` and `writeCrdEnabledToStorage(enabled: boolean): void`, and **invert the default**: when the LS key is unset or unreadable, `useCrdEnabled()` returns `true` (new design). The existing `CRD_TOGGLE_STORAGE_KEY` export stays. Keep the hook synchronous.
...
- The default-inversion in T002 is the highest-impact line of code in this feature: every existing user without an LS key flips to the new design on deploy. This is intentional per spec clarification 2026-05-12, Q3 — do not soften it during review.

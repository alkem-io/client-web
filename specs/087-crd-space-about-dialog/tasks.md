...
- [X] T003 Extend `src/crd/i18n/space/space.en.json` with the full new key set under `about.*` and `apply.*` blocks (see [data-model.md § Translation keys](./data-model.md#translation-keys-new-additions-to-crd-space-namespace) for the exact JSON to merge into the existing `about` block and add as a new sibling `apply` block).
...
- [X] T010 [P] [US2] Create `src/crd/components/community/ApplicationSubmittedDialog.tsx`: compact shadcn `Dialog` (`sm:max-w-md`). Title `t('apply.submitted.title')`, body `t('apply.submitted.body', { communityName })`, single primary `Button` with label `t('about.close')` whose `onClick` calls `onOpenChange(false)`. ≤40 lines.
...
- [X] T015 [US2] Wire the apply flow into `src/main/crdPages/space/about/CrdSpaceAboutPage.tsx` (extends T006). Steps:
...
- [X] T016 [US3] Wire contact-host into `src/main/crdPages/space/about/CrdSpaceAboutPage.tsx` (extends T006/T015). **Failure UX ownership**: the reused MUI `DirectMessageDialog` (returned by `useDirectMessageDialog`) handles its own send-failure UX internally. Do NOT add an additional `useNotification` toast at this layer for direct-message send failures — let the MUI dialog's existing error handling run. The integration container only handles the unauthenticated → sign-up redirect. Steps:
...
- [X] T020 [US5] Wire guidelines into `src/main/crdPages/space/about/CrdSpaceAboutPage.tsx` (extends T006/T015/T016/T017). Steps:
...
- [X] T021 [P] Mirror new keys from `src/crd/i18n/space/space.en.json` into `src/crd/i18n/space/space.nl.json`. Copy English values where Dutch translations don't yet exist (consistent with current `about.*` block convention).
- [X] T022 [P] Mirror new keys into `src/crd/i18n/space/space.es.json`.
- [X] T023 [P] Mirror new keys into `src/crd/i18n/space/space.bg.json`.
- [X] T024 [P] Mirror new keys into `src/crd/i18n/space/space.de.json`.
- [X] T025 [P] Mirror new keys into `src/crd/i18n/space/space.fr.json`.
- [X] T026 Run `pnpm lint` from repo root and resolve any TypeScript / Biome / ESLint findings introduced by this branch.
- [X] T027 Run `pnpm vitest run` from repo root and confirm the existing test suite still passes (no new tests required by this feature; existing 595-test baseline must not regress).
- [X] T028 [P] CRD MUI-cleanliness audit: from repo root, run `grep -rn "@mui\|@emotion" src/crd/components/space/SpaceAbout* src/crd/components/space/CommunityGuidelinesBlock.tsx src/crd/components/community/` — confirm output is empty.
- [X] T029 [P] Integration-layer MUI-cleanliness audit: from repo root, run `grep -rn "@mui\|@emotion" src/main/crdPages/space/about/` — confirm only acceptable matches are the inline comment marking the reused `directMessageDialog` from `useDirectMessageDialog` (no direct `@mui/*` imports).

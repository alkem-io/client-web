...
- **Template "selectable" parity rule.** The MUI selector only lets you pick Space templates whose captured space has a **valid 4-state innovation flow**. The CRD `TemplatePickerSelectProps` has no per-row "disabled/selectable" capability today → **filter non-selectable templates out of the picker's sources at the integration layer** (decision R5), so only valid L0 templates appear. No picker API change.

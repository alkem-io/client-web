...
**Decision**: Only Space templates whose captured space has a **complete (4-state) innovation flow** may seed an L0 Space. This is enforced **on selection, not by pre-filtering the picker** — when the user picks a template, `useCreateSpace.applyTemplate` fetches its content and, if the captured space does not have exactly 4 innovation-flow states, rejects the choice with a clear notification (`template.invalidFlow`), clears the picker selection, and applies nothing.

**Rationale (corrected — review round 6)**: The MUI form enforces this via `isTemplateSelectable` (`template.contentSpace?.collaboration.innovationFlow.states.length === 4`). The *original* plan was to pre-filter non-selectable templates out of the picker's source rows at the integration layer. **That turned out not to be feasible**: the picker's source rows come from the template **cards**, which carry only profile/metadata — they do **not** include the captured space's `collaboration.innovationFlow`, so selectability can't be determined without fetching each template's full content (`includeSpace: true`) up-front for every row. Eagerly fetching content for all templates just to grey out a few is a large, wasteful round-trip on a shared picker. So the rule moved to **post-selection rejection**: a single content fetch already happens on selection (to build the preview and pre-fill fields), and the flow-state check piggybacks on it. The user-visible outcome — an invalid template cannot seed a Space — is the same; the difference is the template is rejected *after* a click rather than hidden up-front.

**Alternatives considered**:
- *Pre-filter at the mapper seam* (the original decision) — rejected: requires fetching full content for every template row (see above).
- *Extend `TemplatePicker` with an `isSelectable`/`disabledIds` capability and render disabled rows* — still needs per-row flow data (same fetch cost) and touches a shared component used by many flows; deferred as a possible future enhancement, not needed for parity.
...
**Dashboard launch (review round 3 — FR-021; refined round 6)**: The CRD dashboard sidebar's `create-space` item (`useDashboardSidebar.ts`) previously set `href: useCreateSpaceLink()` (→ the account page). `useDashboardSidebar` now takes an **optional** `onCreateSpaceClick`: when provided the item uses `onClick: onCreateSpaceClick` (opens the dialog, rendered as a `<button>`); when omitted it **falls back to the original `href: useCreateSpaceLink()`**. Per the reviewer: only the shell that actually had a "Create Space → account" link gets the dialog — `DashboardWithMemberships.tsx` passes `onCreateSpaceClick` and mounts `<CrdCreateSpaceDialog accountId={currentUserAccountId} … />`; `DashboardWithoutMemberships.tsx` is left unchanged (keeps the link fallback). This avoids introducing a new create-space entry point where there wasn't one.
...
**Rationale**: Root-cause fix per the constitution's debugging rule — the earlier `pb-1` only masked the crop and did nothing for the overflow. Trade-off: a native scrollbar instead of the Radix styled one, acceptable for a contained preview.

**R14b — Accordion last-item bottom border (the real cause of the "missing border", corrected).** Separate from the scroll overflow: the `AccordionItem` **primitive** defaults to `cn('border-b last:border-b-0', className)`. In `SpaceTemplatePreview` each item is a self-contained card (`border rounded-lg`), but the primitive's `last:border-b-0` (`:last-child`, specificity 0,1,1) overrides the plain `.border` (0,1,0) and strips the **last** item's bottom border. Fix: add `last:border-b` to the item's className in `SpaceTemplatePreview.tsx` — `tailwind-merge` keeps it (it comes after the primitive default in the merged string), restoring the bottom border. (Earlier note that `.last:border-b-0` was "unrelated" was wrong — it is exactly this primitive default.)

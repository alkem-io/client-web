...
6. **Given** the editor clicks "Settings" in the Edit Callout dialog for a poll, **When** the settings dialog opens, **Then** all settings checkboxes ("Allow multiple responses per user", "Allow users to add new options", "Only show results after a user has voted", "Show avatars of voters in the results") are shown in a disabled read-only state. The gear icon and min/max fields are temporarily hidden.

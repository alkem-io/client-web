...
- [ ] T031 [US5] Add reorder controls (up/down arrow buttons) to `PollFormFields` in `src/domain/collaboration/poll/PollFormFields.tsx` — each option row gets up/down arrow IconButtons for reordering, using Formik FieldArray `swap` helper. Options maintain their order via array position (index maps to sortOrder)

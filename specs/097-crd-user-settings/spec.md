...
### User Story 1 - User My Profile Tab (Priority: P1)
...
- Clicking **Save** fires the per-field mutation immediately. On success the field returns to idle with a transient grayed-out "Saved" indicator next to the field label for ~2 seconds. **On failure the field stays in edit mode with the user's typed value preserved (no auto-revert)**, and an inline error appears beneath the input; the user retries by clicking Save again or discards by clicking Cancel (or pressing Escape) to revert to the last server value.
...
- **Social Links / References**: each existing reference row has the same per-field Save / Cancel affordance. The **Add Another Reference** button creates a new row already in edit mode; clicking Save creates the reference; clicking Cancel removes the unsaved row without a mutation. The trash icon on a saved reference fires delete immediately (no confirmation dialog — references are cheap to recreate).
- **Avatar uploads** commit immediately on file-select (the act of selecting a file is the explicit commit) — they have no separate Save click because the file-picker dialog itself is the confirmation step.

Because there is no tab-wide dirty buffer, **switching tabs never blocks the navigation**. If the user navigates away while a single field is mid-edit, that pending edit is **silently dropped** — no mutation fires, no confirmation dialog appears.
...
3. **Given** a field is in edit mode, **When** the owner clicks Save (or presses Enter on a single-line input), **Then** the per-field mutation fires, the field returns to idle on success with a transient "Saved" indicator next to the label, and the new value is reflected on reload.
4. **Given** a field's per-field save fails, **When** the response returns an error, **Then** the field stays in edit mode with the user's typed value preserved, an inline error appears beneath the input, and the user can click Save again to retry without retyping.
5. **Given** the owner adds a new social reference, **When** they click Save on the new row, **Then** the create-reference mutation fires and the reference appears on the public profile page after reload.
...
The body is the same two-column layout as User My Profile (form on the left, avatar/logo preview on the right). The form is divided into four subsections, each with the icon + bottom-bordered title treatment. **Field set mirrors the current MUI `OrganizationForm` exactly:**
...
- **Social Links / References** — three recognized social references (LinkedIn, Bluesky, GitHub) + arbitrary references list with **Add Another Reference** button — same shape as User My Profile.
...
**Save model — per-field with explicit save**, identical to User My Profile. Each editable field has its own pencil → check / × affordance. Display Name + Description are required (cannot save empty). Domain, Legal Entity Name, Contact Email, Website are optional. Per-field save failures keep the typed value in edit mode with an inline error. There is **no** tab-wide Save / Discard bar — a deliberate UX divergence from MUI's Formik-with-single-Save (`OrganizationForm`), chosen for consistency with User My Profile and the rest of the contributor settings vertical.
...
**Independent Test**: Open `/organization/<orgSlug>/settings/community` as an org admin. Search "Maria" — available users filter. Click + on a user — they move to the associates list (mutation fires immediately). Click × on an existing associate — they're removed.
...
4. **Given** an org admin clicks × on an associate, **When** they click, **Then** the remove-role mutation fires immediately and the user is removed from the associates list.
...
**Independent Test**: Open `/organization/<orgSlug>/settings/authorization` as an org admin. Confirm two sub-tabs (Admin, Owner). On the Admin sub-tab, click + on an available user — they move to the Admin list. Switch to Owner sub-tab — see the org's Owner list. Click × on an Owner — they're removed from the Owner role.
...
4. **Given** an org admin clicks × on an existing Owner, **When** they click, **Then** the remove-role mutation fires and the user is removed from the Owner list.
...
- **My Profile / Org Profile mid-edit + tab switch**: there is no tab-wide dirty buffer. When the user is mid-edit on a single field and navigates away (tab click, link click, browser back), the in-progress value is silently dropped — no confirmation dialog appears.
...
- **Per-field save failure (User My Profile or Org Profile)**: the field stays in edit mode with the user's typed value preserved, and an inline error appears beneath the input. Retry by clicking Save again; discard by clicking Cancel / pressing Escape.
...
- **FR-016**: No settings tab — User My Profile, Org Profile, or any other — carries a tab-wide dirty buffer. Tab strip clicks and out-of-page navigations MUST NOT be blocked by a confirmation dialog. Any field that is mid-edit when the user navigates away has its in-progress value silently dropped.
- **FR-017**: All settings tabs MUST commit changes immediately on per-control confirmation (no tab-wide Save / Reset bar anywhere) — User My Profile and Org Profile use the per-field explicit-save model (FR-020 / FR-090); all other tabs commit per-control on click.

#### User My Profile tab (User Story 1)

- **FR-020**: The User My Profile tab MUST use a **per-field explicit-save** model. Each editable field commits independently via its own Save (check) + Cancel (×) affordance revealed when the field enters edit mode. There is NO tab-wide Save Changes / Discard Changes bar, and there is NO autosave / debounce.
- **FR-021**: The User My Profile tab MUST render the editable fields mirroring the current MUI `UserForm` exactly: Identity (Display Name, First Name, Last Name — required; Email — read-only; Phone — validated), About You (Tagline, City, Country select, Bio markdown, Tags), Social Links (LinkedIn, Bluesky, GitHub recognized refs + arbitrary references list with Add Another Reference). The system MUST NOT surface an Organization input or Twitter / X tile.
- **FR-022**: Each per-field Save click MUST fire one targeted mutation (preserving the rest of the user payload). On success, the field MUST return to idle with a transient "Saved" indicator next to the label for ~2 seconds. **On failure, the field MUST stay in edit mode with the user's typed value preserved** — the system MUST NOT auto-revert to the server value — and an inline error message MUST appear beneath the input. The error MUST persist until the user clicks Save again (retry) or clicks Cancel / presses Escape (discard and revert).
- **FR-023**: A per-field **Cancel** (× icon, or Escape inside the input) MUST revert the field to its last server value with no mutation. **Enter** inside a single-line input MUST be equivalent to clicking Save; the Bio rich-text field is exempt because Enter inserts a newline.
- **FR-024**: Avatar uploads on User My Profile MUST commit immediately on file-select. They have no separate Save click.
- **FR-025**: Reference CRUD (create / update / delete) MUST reuse the existing reference mutations the MUI `UserForm` already wires; no new mutation is added. Deleting a reference fires immediately on trash-icon click with **no** confirmation dialog (references are cheap to recreate).
- **FR-026**: There is no read-only mode on the User My Profile tab. The shell either renders fully editable My Profile fields (when `canEditUserSettings` is true) or has redirected the viewer to the public profile.
...
- **FR-064**: Each toggle MUST use the optimistic-overrides pattern (immediate UI update, then resync after server refetch).
...
- **FR-090**: The Org Profile tab MUST use the **same per-field explicit-save** model as User My Profile (FR-020 — FR-023). This is a deliberate UX divergence from MUI's Formik-with-single-Save pattern in `OrganizationForm`, chosen for consistency with the User vertical.
...
- **FR-092**: Each per-field Save click MUST fire one targeted `updateOrganization` mutation preserving the rest of the org payload. Reference and tagset CRUD reuse the same mutations as User My Profile.
- **FR-093**: Avatar/logo upload on Org Profile MUST commit immediately on file-select (parity with User).
...
- **FR-112**: Add / remove actions MUST commit immediately on click via the existing role-set manager. On error, the row keeps its prior state and an inline toast surfaces the error message.
...
- **FR-121**: Each role-assignment view MUST commit add / remove actions immediately on click via the existing role-set manager.
...
- **FR-153**: All confirmation dialogs (Leave space / Leave organization on User tabs) MUST be implemented with the CRD `Dialog` / `AlertDialog` primitives that already provide the required ARIA semantics.
...
- **SC-001**: A profile owner can complete the full edit flow on User My Profile (change name, tagline, bio, location, add a social link, upload a new avatar, save) in under 90 seconds with no console errors and no failed network calls.
...
- Email change on User remains an identity-provider-managed concern; the User My Profile email field stays read-only with a "Contact support to change email" caption.
...
- The 7-tab User strip order (My Profile → Account → Membership → Organizations → Notifications → Settings → Security) is fixed; tabs cannot be reordered. The 5-tab Org strip order (Profile → Account → Community → Authorization → Settings) is fixed.
...
- **No tab-wide dirty buffer anywhere.** Every settings tab — including User My Profile and Org Profile — commits per-control on explicit user action; switching tabs never triggers a discard-confirm dialog.

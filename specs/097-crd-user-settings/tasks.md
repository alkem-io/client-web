...
- [X] T068 [US9] Implement `CrdOrgAccountTab` at `src/main/crdPages/topLevelPages/organizationPages/settings/account/CrdOrgAccountTab.tsx`. Same callback wiring as `CrdUserAccountTab` (T034) — navigates to existing MUI admin routes for create / manage / delete. Owns the destructive `ConfirmationDialog` for Delete (Rule #9). Reads `organizationId` from `useOrganizationContext`; refetches `AccountInformation` after each delete mutation.
...
- [X] T086 [P] Run `pnpm lint` at the repo root and resolve any TypeScript / Biome / ESLint errors in the new files. **Result**: `pnpm tsc --noEmit` clean; `pnpm biome check` clean on all spec-owned files; full repo lint exits 0 (490 pre-existing warnings repo-wide, none in spec-owned files).
- [X] T087 [P] Run `pnpm vitest run` and ensure all new tests pass alongside the existing suite (target completion ~9 s per CLAUDE.md). **Result**: 99 test files / 1009 passing tests + 3 skipped (pre-existing); duration 13.4 s. New T078 (`useOrgRoleAssignment`, 8 tests) and T072 (`useOrgAssociates`, 6 tests) all green.
- [ ] T088 Run `pnpm analyze` and verify the combined gzipped bundle delta across the two new lazy-loaded settings chunks (User Settings + Org Settings) does NOT exceed +50 KB over the prior build (SC-007). Log the chunk sizes in this task's notes for the PR description. **Deferred to user**: requires a full production build (~20 s) + interactive `build/stats.html` review. Per the user's "I'll do the manual testing ones when you're done" directive.

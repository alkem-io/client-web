...
**Goal**: Detail view renders the banner (when present — no fallback is rendered if `bannerUrl` is undefined), title, author attribution caption, markdown description, tags, references, and a comments column with thread + input when permitted. Skeleton state during load, "not found" state for deleted/invalid events. "Edit" and "Back" header actions.
...
- [X] T021 [US3] Implement `mapCalendarEventDetailsToDetailData` in `/Users/borislavkolev/WebstormProjects/client-web/src/main/crdPages/space/dataMappers/calendarEventDataMapper.ts`: takes `(event, { loading, notFound })` and returns `EventDetailData`. Author maps to `{ id: createdBy.id, name: createdBy.profile.displayName, avatarUrl: createdBy.profile.visual?.uri, profileUrl: createdBy.profile.url }` (no `color` field — the leaf component derives it via the `resolveColor` callback prop, see T022/T024). `createdDate` parsed as `new Date(...)`. Tags from `profile.tagset.tags`. References mapped to `{ id, name, uri, description }`. Banner: `bannerUrl` from `profile.banner?.uri` (no `bannerColor` field for the same reason). Location: `event.profile.location?.city?.trim() || undefined` so blank-only values collapse to `undefined` and the detail view omits the row entirely. When `notFound`, return a stub with `notFound: true` and minimal placeholder strings; the event id flows through unchanged so the component can still derive a fallback colour.
- [X] T022 [P] [US3] Build `/Users/borislavkolev/WebstormProjects/client-web/src/crd/components/space/timeline/EventDetailView.tsx` per `EventDetailViewProps`. The component MUST NOT import `pickColorFromId`; it receives `resolveColor: (id: string) => string` as a prop and invokes it on demand (author-avatar fallback only — events no longer use a gradient fallback for missing banners). Layout at ≥768px: two columns — left optionally renders a banner block when `event.bannerUrl` is present (no fallback element when absent), followed by title, author caption ("by {name} • {createdDate formatted}") with the author avatar (`event.author.avatarUrl` when present; otherwise an `Avatar` fallback whose background is `props.resolveColor(event.author.id)`), markdown description via `<MarkdownContent>`, an optional location row (rendered only when `event.location` is set, with a `MapPin` icon whose `aria-label` is `t('calendar.fields.location')` followed by the city text), tags as `<Badge>` row, references as link list. Right has comments column when `showComments` (header `t('details.comments')` + `commentCount`, then `commentsSlot`, then sticky `commentInputSlot` if non-null). At <768px: stack vertically, comments below body. When `event.loading`: skeleton title + 3 skeleton lines for description. When `event.notFound`: centred `t('notFound.title')` + body (`t('notFound.body')`) + a `<Button onClick={onBack}>{t('notFound.backToList')}</Button>`. The `onBack` prop is required — the connector wires the same callback that powers the dialog header's Back action so both surfaces route through one path.
...
- [X] T036 [P] [US6] Build `/Users/borislavkolev/WebstormProjects/client-web/src/crd/components/space/timeline/icons/CrdAddToCalendarIcons.tsx`. Colocate the brand SVGs in the same folder (`./google.svg`, `./outlook.svg`) so the wrapper has no `@/domain/` import — both files set `fill="currentColor"` on `<path>` so the icon takes its colour from the surrounding text rather than rendering invisibly. Import each via Vite's `?react` plugin and apply `inline-block size-4 text-foreground` (Tailwind) on the wrapped element so the icon is visible against the dropdown background regardless of the parent's inherited colour. Export `{ GoogleCalendarIcon, OutlookCalendarIcon }`. No MUI imports. (The ICS download trigger uses lucide's `Download` icon directly in `AddToCalendarMenu.tsx` — no brand glyph needed.)
...
- [ ] T050 [P] Legacy MUI regression check: in browser console run `localStorage.setItem('alkemio-design-version', '1'); location.reload();`; verify the legacy MUI `CalendarDialog` and `DashboardCalendarSection` still open and operate exactly as before; confirm no files under `src/domain/timeline/calendar/` were modified by this branch.

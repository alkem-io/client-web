...
- [ ] T015 [US2] Implement `mapCalendarEventInfoToListItem` in `/Users/borislavkolev/WebstormProjects/client-web/src/main/crdPages/space/dataMappers/calendarEventDataMapper.ts` returning the full `EventListItem` shape (id, title, description from `profile.description`, raw `startDate: Date`, durationMinutes, durationDays, wholeDay, type as string, url from `profile.url`, subspaceName from `event.subspace?.about.profile.displayName` when present).
...
- [ ] T052 [P] Static checks: run `pnpm lint` (tsc + biome + eslint), `pnpm vitest run`, and `pnpm build` (production) from the repo root; all three must pass with zero new errors. Fix any TypeScript issues that surface from the new types in `contracts/`.

...
- [X] T001 Confirm working branch is `100-space-header-layout` and tree is clean (`git status` shows no uncommitted changes that aren't part of this work)
- [X] T002 [P] Run `pnpm install` to ensure dependencies are current with the lockfile
...
- [X] T007 [US2] Rewrite `src/crd/components/space/SpaceHeader.tsx` markup to add the new below-banner section per `specs/100-space-header-layout/data-model.md` (`# SpaceHeader → Markup contract`). Inside the new section, render title (`text-hero text-foreground truncate`) and optional subtitle (`text-body text-muted-foreground truncate`). Keep the `isHomeSpace` Home glyph but switch its colour from `text-primary-foreground/80` to `text-muted-foreground`. **Remove** from this file: the in-banner title/tagline overlay block (lines ~164–182), the member-avatar stack block (lines ~185–215). Leave the action-buttons block (lines ~84–162) untouched for now — US3 owns that move.
- [X] T008 [US2] Remove unused props from `SpaceHeader` prop type in `src/crd/components/space/SpaceHeader.tsx`: drop `memberAvatars: MemberAvatar[]` and `onMemberClick?: () => void` from `SpaceHeaderProps`. Drop the `MemberAvatar` local type if it has no other use. Update the destructuring in the function signature accordingly.
- [X] T009 [US2] In `src/main/crdPages/space/layout/CrdSpacePageLayout.tsx`, stop passing `memberAvatars` and `onMemberClick` to `<SpaceHeader>`. Remove any local computations that only fed those props. _Also removed the now-orphaned `communityOpen` state and `CrdSpaceCommunityDialogConnector` mount that were only triggered by `onMemberClick`._
- [X] T010 [US2] In `src/crd/app/pages/SpacePage.tsx` (standalone preview), trim the mock props passed to `<SpaceHeader>` — drop `memberAvatars` and `onMemberClick`.
- [X] T011 [US2] Rewrite `src/crd/components/space/SubspaceHeader.tsx` markup to add the new below-banner section per `specs/100-space-header-layout/data-model.md` (`# SubspaceHeader → Markup contract`). The title row contains: a single 56px (`size-14`) subspace avatar (using `subspaceAvatarUrl` when present, else initials over `subspaceColor`) → title (`text-hero text-foreground truncate`) → (button group placeholder kept for now until T016). Subtitle row (`text-body text-muted-foreground truncate`) below when `tagline` is set. **Remove** from this file: the two-tile layered avatar block (lines ~234–263) and its `-mt-12` overlap, the level Badge (lines ~184–194), the member-avatar stack block (lines ~196–227). Leave the in-banner action-buttons block (lines ~102–182) untouched until US3.
- [X] T012 [US2] Remove unused props from `SubspaceHeader` prop type in `src/crd/components/space/SubspaceHeader.tsx`: drop `parentInitials`, `badgeKind`, `memberAvatars`, `onMemberClick`, and `parentName` (the banner `aria-label` uses `title` not `parentName`). Drop the `MemberAvatar` local type. _Implementation deviation: `parentColor` was **kept** (not dropped) because FR-009 requires the existing deterministic gradient fallback when `parentBannerUrl` is missing — `parentColor` is the colour input for that gradient. Spec FR-013 listed `parentColor` for removal; that was an oversight relative to FR-009. Keeping `parentColor` is the right call._
- [X] T013 [US2] In `src/main/crdPages/subspace/layout/CrdSubspacePageLayout.tsx`, stop passing `memberAvatars` and `onMemberClick` to `<SubspaceHeader>`. Updated `subspacePageDataMapper.ts` `SubspaceBannerProps` and `mapSubspaceBanner()` to drop `parentName`, `parentInitials`, `badgeKind` from the output (kept `parentColor`). Updated `useCrdSubspace.ts` to drop `bannerAvatars` from the return type and the `mapMemberAvatars` import.
- [X] T014 [US2] In `src/crd/app/pages/SubspacePage.tsx` (standalone preview), trim the mock props passed to `<SubspaceHeader>` — drop `parentName`, `parentInitials`, `badgeKind`, `memberAvatars`, `onMemberClick`; drop the orphaned `MEMBER_AVATARS` constant. Kept `parentColor` (per the FR-009 carve-out noted in T012).
...
- [X] T017 [P] Prune orphaned i18n keys from `src/crd/i18n/space/space.{en,nl,es,bg,de,fr}.json`. Removed `members.title` from all six files. Verified via `git grep` — only the now-removed SpaceHeader call site referenced it.
- [X] T018 [P] Prune orphaned i18n keys from `src/crd/i18n/subspace/subspace.{en,nl,es,bg,de,fr}.json`. Removed the entire `badge` block (`subspace`, `subSubspace`), `members.viewCommunity`, and `a11y.parentLink` from all six files.
- [X] T019 Ran `pnpm lint` — no errors introduced by this change. The only remaining errors are pre-existing in unrelated files (non-null assertions in `useSpaceExplorerViewState.ts` / `useConversationMessages.ts`, banned-type in `StandalonePlatformNavigationMenu.tsx`) and the gitignored `build/meta.json` formatting nit. TypeScript (`npx tsc --noEmit`) passes cleanly. React Compiler rule satisfied — no manual `useMemo`/`useCallback`/`React.memo` introduced.
- [X] T020 Ran `pnpm vitest run` — **1123 tests passed, 3 skipped, 111 files passed**. Includes the `space.parity.test.ts` after pruning `members.title` from all six space files.
- [ ] T021 Run the full manual verification plan in `specs/100-space-header-layout/quickstart.md` — every row of every table (US1, US2, US3, a11y spot checks, removed-features regression, constitutional checks) at viewports 320/768/1024/1440/1920 in light and dark mode, on both the standalone preview app and the integration app. **(User action — manual visual QA)**
- [ ] T022 Side-by-side visual review (Space vs Subspace) at three viewports (mobile, desktop, wide). Confirm SC-007: identical banner width, banner aspect ratio, title placement, subtitle placement, and action button placement. **(User action — manual visual QA)**

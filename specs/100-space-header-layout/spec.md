...
|-------|-------------|----------|
| `banner` (Visual type `BANNER`, ≥1536px wide at an admin-chosen 6:1–10:1 ratio, 10:1 default — *was ~1920×320 fixed 6:1*) | **L0 only** | The full-width page banner on a space's page layout — on top of the L0 page, **and** on top of L1/L2 pages (which inherit it from the L0 root of their ancestry chain). |
...
| `avatar` (Visual type `AVATAR`) | **L1 and L2 only** | The small inset image for an L1/L2. Used in any list-style display of an L1/L2 (sidebar lists, admin Subspaces rows, notification icons, activity-item icons) and in the breadcrumb trail. *(Amended 2026-08-21: no longer rendered inline in the L1/L2 page header — the header avatar tile was removed; breadcrumbs gained space/subspace identity images instead.)* |
...
- **L1/L2 page header** → `banner` (inherited from the **L0 root**, not the immediate parent for L2) + title. *(Amended 2026-08-21: the inline `avatar` was removed from the page header.)*
...
- **FR-009a**: The banner MUST scale fluidly with viewport width at the width / height ratio stored on the space's BANNER visual (`Visual.aspectRatio`, bounded by the server to 6–10 and chosen by the admin in the Settings > About crop dialog's shape slider), falling back to the 10:1 design default (`DEFAULT_BANNER_ASPECT_RATIO`) when there is no image or the value has not loaded. Today's fixed `h-[256px]` (Space) and `h-52 md:h-64` (Subspace) are replaced. *(Amended 2026-08-31 — originally a fixed `aspect-ratio: 6 / 1`; PR #10121 made the ratio per-space and PR #10222 / issue #10178 moved the default to 10:1. See A8.)*
...
- **FR-013**: Several existing props become unused as a result of the layout change:
  ...
  Each of these may be removed from the prop type in this change or kept as accepted-but-ignored. Consumers in `src/main/crdPages/` stop passing them either way. All other existing props (`title`, `tagline`, `bannerUrl`, `color`, `isHomeSpace`, `actions`, `parentBannerUrl`) carry over unchanged. *(Amended 2026-08-21: `subspaceInitials`, `subspaceColor`, and `subspaceAvatarUrl` — originally in the carry-over list — were deleted along with the header avatar tile.)*

### Key Entities

This is a presentational layout change — no new data entities or props are introduced. Existing props on `SpaceHeader` (`title`, `tagline`, `bannerUrl`, `color`, `isHomeSpace`, `actions`) and `SubspaceHeader` (`title`, `tagline`, `parentBannerUrl`, `actions`) remain *(the subspace identity trio was later removed — see the FR-013 amendment)*. Their meaning is unchanged; only their visual arrangement changes. Props made unused by this change (`memberAvatars`, `onMemberClick`, `SubspaceHeader.parentInitials`, `SubspaceHeader.parentColor`, `SubspaceHeader.badgeKind`) are governed by FR-013.
...
- **A7 — Subspace avatar simplifies to a single tile** (clarified 2026-05-14; **superseded 2026-08-21**). The current two-tile layered avatar (parent behind, subspace in front, `-mt-12` upward overlap into the banner) is replaced by a single ~56px subspace avatar inline with the title row, matching the prototype's `SubspaceHeader.tsx`. The `-mt-12` upward-overlap pattern is removed; the avatar sits cleanly in the below-banner section alongside the title and subtitle. *Superseding decision: the single inline avatar was subsequently removed too — the L1/L2 page header now renders the bare title (no avatar tile at any level, matching L0), and subspace identity images moved into the breadcrumb trail.*
- **A7a — Subspace page banner uses the L0 root's banner image** (final decision). Per the "Visual fields — canonical usage" rule above, the subspace page banner is sourced from `levelZeroProfile.banner?.uri` (the L0 ancestor's BANNER visual, stored at page-banner resolution — ≥1536px wide at the space's own 6:1–10:1 ratio, e.g. 1920×192 at the 10:1 default or 1920×320 at 6:1). For an L1, the L0 ancestor is the immediate parent; for an L2 (sub-sub-space), the L0 ancestor is the grandparent — *not* the immediate L1 parent (L1 has no settable page banner). The subspace's own `cardBanner` is **not** used because cardBanner is 416×256 (sized for cards) and would be visibly blurry stretched to a full-width banner. When the L0 root has no banner uploaded, the deterministic gradient fallback uses the L0's colour (`pickColorFromId(levelZeroSpaceId)`). Subspace identity is conveyed via the breadcrumb trail (which since 2026-08-21 shows the subspace's avatar as a 20px rounded square) and the subspace title and tagline *(the inline ~56px header avatar was removed the same day)*. **Iteration history**: an interim attempt to source from the subspace's own image produced unacceptable blur; a follow-up that used the *immediate* parent (instead of L0) failed on L2 because L1 has no banner. The L0-rooted lookup matches the legacy MUI `SubspacePageBanner.tsx` (`lookup.level0Space.about.profile.banner`). A future platform change that exposes a high-res BANNER visual type on L1/L2 subspaces could let this assumption be revisited.
- **A8 — Banner height is fluid via the per-space banner ratio** (clarified 2026-05-14 as a fixed `aspectRatio: 6 / 1`; **superseded 2026-08-31**). The banner scales with viewport width — wider viewports get a proportionally taller banner. This replaces today's fixed `h-[256px]` (Space) and `h-52 md:h-64` (Subspace). *Superseding decision: the ratio is no longer a fixed 6:1 Tailwind class. It is the value stored on the space's BANNER visual (`Visual.aspectRatio`, server-bounded 6–10 via `DEFAULT_VISUAL_CONSTRAINTS[BANNER]`, set by the admin in the Settings > About crop dialog — PR #10121). `SpaceHeader` renders it as the banner `<img>`'s `width`/`height` attributes (`bannerPlaceholderSize`, reserving a correctly shaped box before load while the loaded image's own height wins) and as the inline `aspectRatio` of the gradient fallback; `SubspaceHeader` does the same with the L0 banner's ratio. The design default is 10:1 (`DEFAULT_BANNER_ASPECT_RATIO`, PR #10222 / issue #10178); the server's 6:1 row-creation default is ignored until an image exists (`resolveBannerAspectRatio`, `src/crd/lib/bannerAspectRatio.ts`).*

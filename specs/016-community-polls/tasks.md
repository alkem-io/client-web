...
- [x] T017 [US1] Create `CalloutFramingPoll` component in `src/domain/collaboration/callout/CalloutFramings/CalloutFramingPoll.tsx` — framing display component following the same pattern as `CalloutFramingWhiteboard.tsx` and `CalloutFramingMemo.tsx`. Props: `callout: CalloutDetailsModel`. Returns null if `callout.framing.poll` is undefined. Determines `canVote` from `callout.authorization?.myPrivileges` (checks for CONTRIBUTE privilege) and `editable` from callout editability. Renders `PollView` with the poll data and permission flags
...
**Goal**: Callout editors can add, edit, remove, and reorder poll options. Destructive actions (edit/remove) that affect votes show confirmation dialogs.

**Independent Test**: As a facilitator, enter poll edit mode → add an option → edit an option's text → remove an option → reorder options → verify all changes persist.

### Implementation for User Story 5

- [x] T030 [P] [US5] Create `usePollOptionManagement` hook in `src/domain/collaboration/poll/hooks/usePollOptionManagement.ts` — wraps the generated mutations: `useAddPollOptionMutation`, `useUpdatePollOptionMutation`, `useRemovePollOptionMutation`, `useReorderPollOptionsMutation`. Accepts pollID. Returns `{ addOption(text), updateOption(optionID, text), removeOption(optionID), reorderOptions(optionIDs), loading }`. Each mutation returns the full updated Poll via `...PollDetails` fragment for automatic cache update
- [x] T031 [P] [US5] Create `PollOptionManagerRow` component in `src/domain/collaboration/poll/PollOptionManagerRow.tsx` — displays a single option in edit mode. Props: option (PollOptionModel), onEdit callback, onRemove callback, canRemove (boolean, false when only 2 options remain), index (for drag handle or up/down buttons). Renders: text display (or inline text field when editing), edit button (pencil icon), remove button (delete icon, disabled when `canRemove` is false with tooltip `poll.options.minRequired`), drag handle or up/down buttons for reordering
- [x] T032 [US5] Create `PollOptionManager` component in `src/domain/collaboration/poll/PollOptionManager.tsx` — full option management UI. Props: poll (PollDetailsModel), pollID (string). Uses `usePollOptionManagement` hook. Renders list of `PollOptionManagerRow` components for each option. Includes "Add Option" button at the bottom. Handles reordering via up/down buttons (calls `reorderOptions` with full option ID list in new order). Before calling `updateOption` or `removeOption`, checks if the option has `voteCount > 0` and shows a MUI confirmation dialog with `poll.options.confirmEdit` or `poll.options.confirmRemove` i18n key
- [x] T033 [US5] Integrate `PollOptionManager` into `PollView` in `src/domain/collaboration/poll/PollView.tsx` — when the `editable` prop is true, show an "Edit Poll" button (`poll.manage.editPoll`). Clicking it toggles an `isManaging` local state that renders `PollOptionManager` in place of (or alongside) the voting/results UI. Add a "Done" button to exit management mode

**Checkpoint**: Option management is fully functional. Facilitators can maintain polls post-creation with appropriate safeguards for vote-affecting changes.
...
- US5: T030, T031 can run in parallel (hook + leaf component)

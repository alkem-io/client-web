...
// MUI system props that map to CSS — translated when spread onto our primitives.
...
/**
 * Split an arbitrary props bag into (a) a CSS style object for the MUI system
 * props it contains, translated MUI-faithfully, and (b) the remaining DOM
 * props. Lets the typography primitives keep accepting the `Box`/`Typography`
 * system-prop API their importers rely on, without an MUI dependency.
 */
export const extractSystemProps = (
  ...
  return { style: style as CSSProperties, rest };
};

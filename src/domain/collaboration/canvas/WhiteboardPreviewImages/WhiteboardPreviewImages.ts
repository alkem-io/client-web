...
import { CanvasWithValue } from '../containers/CanvasValueContainer';
...
export interface WhiteboardPreviewImage {
  // CARD => bannerNarrow for cards, BANNER => Big preview for SingleCanvasCallouts
...
export const generateWhiteboardPreviewImages = async <Canvas extends CanvasWithValue>(
  canvas: Canvas,
  ...
  previewImages.push({
    ...
    imageData: await exportToBlob({
      ...
      elements,
      files: files ?? null,
      getDimensions: getWhiteboardPreviewDimensions(canvas?.profile?.preview ?? BannerDimensions),
      mimeType: 'image/png',
    }),
  });
  ...
  previewImages.push({
    ...
    imageData: await exportToBlob({
      ...
      elements,
      files: files ?? null,
      getDimensions: getWhiteboardPreviewDimensions(canvas?.profile?.visual ?? BannerNarrowDimensions),
      mimeType: 'image/png',
    }),
  });
    ...
    uploadVisuals: async (
      ...
      if (visualsIds.cardVisualId) {
        ...
        if (previewImage) {
          await uploadVisual({
            variables: {
              file: new File([previewImage.imageData], `/Canvas-${canvasNameId}-bannerNarrow.png`, {
                type: 'image/png',
              }),
              uploadData: {
      ...
      if (visualsIds.previewVisualId) {
        ...
        if (previewImage) {
          await uploadVisual({
            variables: {
              file: new File([previewImage.imageData], `/Canvas-${canvasNameId}-banner.png`, { type: 'image/png' }),
              uploadData: {

...
import { contributionIcons } from '../../icons/calloutIcons';
...
const CalloutContributionPreview = ({ callout, contributionId, onClose }: CalloutContributionPreviewProps) => {
  ...
  const contributionType = data?.lookup.contribution?.post ? CalloutContributionType.Post
    : data?.lookup.contribution?.link ? CalloutContributionType.Link
      : data?.lookup.contribution?.whiteboard ? CalloutContributionType.Whiteboard
        : undefined;
  const Icon = contributionType ? contributionIcons[contributionType] : undefined;

  const displayName =
    loading ? <Skeleton variant="text" width={gutters(10)(theme)} /> : (
      (contributionType === CalloutContributionType.Post && data?.lookup.contribution?.post?.profile.displayName) ||
      (contributionType === CalloutContributionType.Whiteboard && data?.lookup.contribution?.whiteboard?.profile.displayName));

  const author = (contributionType === CalloutContributionType.Post && data?.lookup.contribution?.post?.createdBy?.profile.displayName) ||
    (contributionType === CalloutContributionType.Whiteboard && data?.lookup.contribution?.whiteboard?.createdBy?.profile.displayName);

  const formattedCreatedDate =
    (contributionType === CalloutContributionType.Post && data?.lookup.contribution?.post?.createdDate &&
      formatDateTime(data.lookup.contribution.post.createdDate)) ||
    (contributionType === CalloutContributionType.Whiteboard && data?.lookup.contribution?.whiteboard?.createdDate &&
  ...
  return (
    <Gutters>
      <PageContentBlock>
        <PageContentBlockHeaderCardLike icon={Icon} title={displayName} subtitle={author} actions={
          <>
        ...
        {contributionType === CalloutContributionType.Whiteboard &&
          <Box padding={gutters()} margin={gutters(-1)}>
            <WhiteboardPreview
              ...
              onClick={() => data?.lookup.contribution?.whiteboard?.profile.url && navigate(data?.lookup.contribution?.whiteboard?.profile.url)}  //!! this is not correct
              onClose={onClose}
            />
          </Box>
        }

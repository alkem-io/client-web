...
const FilterByTagButtons = <Option extends string>({ value, config, onChange }: FilterByTagButtonsProps<Option>) => {
  ...
  return (
    <>
      <StyledFilterList>
        <Caption>{t('spaces-filter.current-filter')}</Caption>
        {showAll && <FilterChip selected>{t(`spaces-filter.${showAllKey}` as const)}</FilterChip>}
        {showOther && (
          <FilterChip selected canRemove onClick={handleRemoveOther}>
            {t(`spaces-filter.${otherKey}` as const)}
          </FilterChip>
        )}
      ...
      <StyledFilterList>
        ...
        {!showOther && <FilterChip onClick={handleShowOther}>{t(`spaces-filter.${otherKey}` as const)}</FilterChip>}
        {!showAll && <FilterChip onClick={handleShowAll}>{t(`spaces-filter.${showAllKey}` as const)}</FilterChip>}
      </StyledFilterList>
    </>
  );
};

...
import {
  ...
  FileId,
  InitializedExcalidrawImageElement } from '@alkemio/excalidraw/types/element/types';
import {
  BinaryFileData,
...
export const encodeFilesForUpload = async ({
  ...
  for (const [id, fileData] of files) {
    ...
    const encodedFile = buffer; /*await compressData<BinaryFileMetadata>(buffer, {
      encryptionKey,
      metadata: {
        id,
    ...
    if (buffer.byteLength > maxBytes) {
      throw new Error(
        `errors.fileTooBig maxSize: ${Math.trunc(maxBytes / 1024 / 1024)}MB`
      );
    }

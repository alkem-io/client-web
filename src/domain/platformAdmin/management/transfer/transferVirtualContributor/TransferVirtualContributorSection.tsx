...
import useTransferVirtualContributor from './useTransferVirtualContributor';

const T_PREFIX = 'pages.admin.transferVc';

const TransferVirtualContributorSection = () => {
  const { t } = useTranslation();
  const [confirmDialogOpen, setConfirmDialogOpen] = useState(false);
  const [targetAccountId, setTargetAccountId] = useState<string | undefined>();

  const { vc, currentAccountName, loading, transferLoading, error, handleResolve, handleTransfer } =
...
export default TransferVirtualContributorSection;

...
const InviteExistingUserDialog = ({
  ...
  const initialValues: InviteContributorsData = {
    ...
    welcomeMessage: t('community.invitations.defaultInvitationMessage', { space: spaceDisplayName }) as string,
  };
  ...
  const alreadyAppliedTranslation = t('community.invitations.inviteExistingUserDialog.selector.alreadyApplied');
  const alreadyInvitedTranslation = t('community.invitations.inviteExistingUserDialog.selector.alreadyInvited');
  const alreadyMemberTranslation = t('community.invitations.inviteExistingUserDialog.selector.alreadyMember');
  ...
  return (
    <Dialog open={open} fullWidth maxWidth="md">
      ...
      <Gutters paddingTop={subtitle ? 0 : undefined} sx={{ border: '1px solid red' }}>
        {' '}
        {/* //!! */}
            ...
            <Form noValidate autoComplete="off">
              <Gutters disablePadding flexDirection={{ xs: 'column', sm: 'row' }} alignItems={'flex-start'}>
                ...
                <Gutters disablePadding row alignItems={'center'}>
                  <Caption style={{ whiteSpace: 'nowrap' }}>{t('community.invitations.inviteToRole')}</Caption>

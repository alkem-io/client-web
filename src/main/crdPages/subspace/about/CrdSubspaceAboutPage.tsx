...
/**
 * CRD subspace (L1/L2) About page.
 *
 * Counterpart to `CrdSpaceAboutPage` for subspaces. Reads from `SubspaceContext`
 * (so it shows the subspace's own about content) and uses `SpaceContext` only
 * to know the parent L0 URL for the close-fallback.
 *
 * Close behaviour mirrors the legacy MUI `SubspaceAboutPage`: when the viewer
 * has no read access, closing the dialog hard-navigates to the parent space URL
 * instead of going back in history — otherwise the protected-routes guard would
 * re-fire on the previous subspace URL and trap the user in a redirect loop.
 */
export default function CrdSubspaceAboutPage() {
  const { subspace, permissions, loading: subspaceLoading } = useSubSpace();
  const {
    space: {
      about: {
  ...
  const handleClose = () => {
    navigate(permissions.canRead ? profileUrl : parentSpaceUrl);
  };

...
/**
 * Platform role set, the 3 `Feature …` roles — `assignPlatformRoleToUser` /
 * `assignPlatformRoleToOrganization` and their removals for `FEATURE_BETA_TESTER`,
 * `FEATURE_VIRTUAL_ASSISTANT` and `FEATURE_ORGANIZATION_CREATOR`.
 *
 ...
 * above, the 3 `Feature …` roles on `FEATURE_ROLE_ASSIGN` (held by a Platform Users Admin,

...
export function InviteMembersDialog({
  ...
  const handleSendEmail = async () => {
    ...
    if (!trimmed) {
      setEmailError(t('community.invite.email.errors.empty'));
      return;
    }
    if (!EMAIL_RE.test(trimmed)) {
      setEmailError(t('community.invite.email.errors.invalid'));
      return;
    }
    ...
    try {
      ...
      setEmailError(t('community.invite.email.errors.failed'));
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl md:max-w-2xl max-h-[85vh] overflow-y-auto [&>*]:min-w-0">
        <DialogHeader>
          <DialogTitle>{t('community.invite.title')}</DialogTitle>
          <DialogDescription>{t('community.invite.description')}</DialogDescription>
        </DialogHeader>

        <div className="flex flex-col gap-5 py-2">
          {/* Invite by email */}
          <div className="flex flex-col gap-2">
            <h3 className="text-card-title">{t('community.invite.email.title')}</h3>
            <div className="flex gap-2">
              <Input
                ...
                placeholder={t('community.invite.email.placeholder')}
                disabled={inviting}
            ...
            {emailInvitedNotice && !emailError && (
              <p className="text-caption text-emerald-600 inline-flex items-center gap-1">
                ...
                {t('community.invite.email.sent', { email: emailInvitedNotice })}
              </p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-3">
            <h3 className="text-card-title">{t('community.invite.users.title')}</h3>
                  ...
                  {loading && candidates.length === 0 ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={2} className="py-6 text-center text-muted-foreground text-caption">
                        {t('community.invite.users.empty')}
                      </TableCell>
                    </TableRow>
                  ) : (
                    candidates.map(c => {
                          ...
                          <TableCell className="text-right">
                            {isAdded ? (
                              <span className="inline-flex items-center gap-1 text-caption text-emerald-600">
                                <Check aria-hidden="true" className="size-3" />
                                {t('community.invite.users.invited')}
                              </span>
                            ) : (
                              <Button
                                type="button"
                              ...
                              >
                                {isAdding ? (
                                  ...
                                  <>
                                    ...
                                    {t('community.invite.users.invite')}
                                  </>
                                )}
                              </Button>
                            )}

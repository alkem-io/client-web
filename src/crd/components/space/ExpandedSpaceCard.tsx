...
import { clampExcerptSource, hasVisibleExcerptText } from '@/crd/lib/markdownExcerpt';
...
export type ExpandedSpaceCardProps = {
  ...
  /**
   * Per-field excerpt visibility, when the caller already computed it (e.g.
   * `SpaceSubspacesList` parses What/Why/Who once to decide which card variant to
   * render, and hands the result down here instead of the card parsing them again).
   * Optional — omit it to let the card compute its own visibility, so it still works
   * standalone (design-review previews, tests, other future consumers).
   */
  ...
  return (
    ...
    >
      ...
      {/* Full-width footer — closes the card in both the row and the stacked
          arrangement. Always renders (it hosts the call-to-action cue);

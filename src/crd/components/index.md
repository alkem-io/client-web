...
CRD "whiteboard disconnected / collaboration stopped" dialog. Title + online/offline message + optional last-saved line (all props), plus a Reconnect button and a spinner while reconnecting. No "Ok" button — dismissal is the `Dialog`'s built-in close (X) / outside-click / Escape. Rendered at `z-[70]` to stack over the editor shell. Wired via the shared `CollaborativeExcalidrawWrapper`'s optional `renderDisconnectNotice` slot; the provider owns automatic retry. Recovery changes follow the repository's authoritative hierarchy: [constitution](../../../.specify/memory/constitution.md) → [agent guidance](../../../agents.md) → [Copilot instructions](../../../.github/copilot-instructions.md).

...
## Active Technologies
- TypeScript 5.x, React 19 (React Compiler enabled — no manual `useMemo`/`useCallback`/`React.memo`) + Apollo Client (generated hooks only), shadcn/ui + Tailwind v4 + Radix UI (CRD layer `@/crd/*`), `react-i18next`, `lucide-react`. **No new runtime dependencies.** (112-l0-additional-tabs)
- N/A (server-side GraphQL; client uses the Apollo normalized cache). Persistence is via the innovation-flow GraphQL mutations already in use for subspaces. (112-l0-additional-tabs)

...
## Active Technologies
- TypeScript 5.x / React 19 (with React Compiler) + shadcn/ui (Radix UI + Tailwind v4), `lucide-react`, `react-i18next` (existing), `react-router-dom` (existing — used only in the integration layer, never in `src/crd/`), `class-variance-authority` (existing). No new dependencies. (095-crd-auth-error-page)
- N/A (presentation + routing decision; no persistence; no schema changes) (095-crd-auth-error-page)
